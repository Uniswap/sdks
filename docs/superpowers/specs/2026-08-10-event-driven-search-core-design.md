...
The search is an **anytime algorithm**: at every moment there is a current best answer plus an honesty report, monotonically improving as knowledge arrives. It is built from **one loop, two convergence processes, and a verifier**. There are no waves, tiers, stages, task objects, or timers.
...
async function* search(ctx, req, kind): AsyncGenerator<EngineEvent> {
  ...
  sources.launch(validateHints(...), readiness(...))   // bounded
  coverage.demand(BOUNDED_SLICE)                        // recent pair window — bounded
  sources.launch(coverageWorker(...))

  try {
    while (true) {
      await wake.next()                          // coalescing: N pokes before await = 1 wake
      await pump(state, ctx, req, wake)          // §3.2
      maybeVerify(state, ctx, req, wake)         // swaps only, §3.4
      yield* emitChanges(state)                  // lead / progress, coalesced per cycle
      if (state.aborted || allConverged(state, sources)) { yield finalEvent(state); return }
      if (pumpDry(state) && !state.fullDemandOpened) {
        coverage.demand(FULL)                    // the one cost gate
        state.fullDemandOpened = true
      }
      // "consumer still pulling" is structural, not a condition: the loop body only runs
      // between pulls of the generator, so a consumer that stopped never opens the gate.
    }
  } finally {
...
- The **notifier** is the only wake mechanism: poked by source progress (the existing `onLogs` chunk seam), source settlement, measurement-round settlement, preflight settlement, and abort. It coalesces (many pokes before the next `await` produce one wake). No timers exist anywhere in the engine.
...
**Relevant pairs.** The direct pair (in, out), plus (in, X) and (X, out) for each selected intermediate X. Intermediates = hinted tokens, then manifest cores, then discovered neighbor-intersection nodes (newest-touching-pool first), capped by `MAX_INTERMEDIATES` and reported via `intermediatesPruned`. This cap is the one surviving selection heuristic; per-pair pool selection does not survive.

**Measurable legs.** For a relevant pair: every pool the index knows on it, plus **hypotheses** — pool identities derivable without discovery (v2/v3 CREATE2 addresses at standard and fee-scan-discovered tiers, v4 standard configs). Hypotheses reuse the modules' existing `speculativeDirect` enumeration (renamed `hypotheses`). A successful measurement upserts the pool; a data-less revert negative-caches it and feeds hint-discredit history — the existing rules, now on one channel. Speculative direct probes, discovery probes, core-leg probes, and fee-widened probes cease to exist as separate channels: **everything is a measurement.**
...
- **Demand**: per scope this trade needs — adjacency for both endpoints per enabled protocol, the exact-pair scope, the fee-factory scope — the range `[deployBlock, pinnedHead]`. Demand has two urgency classes: the exact-pair scope's recent window (~a week of the chain's blocks — `wave0PairScanBlocks`, unchanged) is **bounded** and demanded from search start; everything else is **unbounded** and demanded only when the loop opens the cost gate.
- **Have**: the shared index coverage cache.
- **Converge**: loop — compute `uncovered = demand − have`, plan merged requests over it (the existing `adjacencyPlan` merging logic, unchanged), execute via `scanLogs` (unchanged, including width memory, declared-cap handling, and backoff), ingest logs chunk-by-chunk poking `wake`, record coverage — and pass again **while the previous pass made progress**. Settle when converged, when a pass covers nothing new (→ discovery `failed`, today's semantics), or when aborted. Widening demand (the loop opening the cost gate) re-wakes a settled worker — convergence is against current demand, not the demand at launch. Within a search the worker tracks ranges it already attempted, so later passes ask only true gaps and never re-ask the reorg tail (today's `adjacencyScanned` bookkeeping, now private to the worker).

The while-progress rule replaces both the fixed retry count (wave 3) and `FEE_DISCOVERY_MAX_REQUESTS`: the fee scan no longer runs *ahead* of adjacency in a serial order it could starve — all unbounded scopes converge concurrently, metered by the shared semaphore and `scanLogs`' own per-scan request budget.

Report derivation is direct: `complete` = demand fully covered for both endpoints; `partial` = gaps remain; `failed` = a zero-progress pass occurred; coverage bars read the index exactly as today (cumulative, endpoint-intersected).
...
1. Two-hop coverage improves: the 3×3 heuristic cross-product becomes exhaustive-by-dominance measurement; warm and cold searches converge to the same answer by construction.
2. RPC volume shifts: more inner calls on dense pairs (all pools measured, batched), fewer on fan-out (leg dedup replaces per-candidate rounds); unbounded scans may start while a straggling bounded source is still running, and fee/adjacency scans run concurrently — the semaphore is the meter.
...
- **New property tests** (fast-check): composition dominance (composed best ≥ every measured combination), measurement dedup (no duplicate (pool, direction, amount) calls in a search), `m_X` invalidation convergence, counter conservation as a fold property.
