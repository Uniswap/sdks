...
export class DutchLimitOrderTrade<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType> {  
    ...
    private getFirstNonFeeOutputStartEndAmounts(): {
      ...
      const output = this.order.info.outputs.find((output) => !output.isFeeOutput)
      if (!output) {
