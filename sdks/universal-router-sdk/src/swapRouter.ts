...
export abstract class SwapRouter {
	...
	public static migrateV3ToV4CallParameters(
		...
		const v4PositionManagerAddress =
			...
			CHAIN_TO_ADDRESSES_MAP[v4Pool.chainId as SupportedChainsType]
		...
		if (v4Pool.currency0.isNative) {
			invariant(
				(v4Pool.currency0.wrapped.equals(v3Token0) &&
					v4Pool.currency1.equals(v3Token1)) ||
					(v4Pool.currency0.wrapped.equals(v3Token1) &&
						v4Pool.currency1.equals(v3Token0)),
				"TOKEN_MISMATCH",
			);
		} else {
		...
		invariant(
			options.v3RemoveLiquidityOptions.liquidityPercentage.equalTo(
				new Percent(100, 100),
			),
			"FULL_REMOVAL_REQUIRED",
		);
		invariant(
			options.v3RemoveLiquidityOptions.burnToken == true,
			"BURN_TOKEN_REQUIRED",
		);
		invariant(
		...
		if (options.v4AddLiquidityOptions.createPool) {
			...
			planner.addCommand(CommandType.V4_INITIALIZE_POOL, [
				poolKey,
				v4Pool.sqrtRatioX96.toString(),
			]);
		...
		if (options.v3RemoveLiquidityOptions.permit) {
			...
			invariant(
				universalRouterAddress ==
					options.v3RemoveLiquidityOptions.permit.spender,
				"INVALID_SPENDER",
			);
			// don't need to transfer it because v3posm uses isApprovedOrOwner()
			encodeV3PositionPermit(
				planner,
				options.v3RemoveLiquidityOptions.permit,
				options.v3RemoveLiquidityOptions.tokenId,
			);
		...
		// encode v3 withdraw
		const v3RemoveParams: MethodParameters =
			V3PositionManager.removeCallParameters(
				options.inputPosition,
				options.v3RemoveLiquidityOptions,
			);
		const v3Calls: string[] = Multicall.decodeMulticall(
			v3RemoveParams.calldata,
		);
		...
		for (const v3Call of v3Calls) {
			...
			invariant(
				selector == V3PositionManager.INTERFACE.getSighash("collect") ||
					selector ==
						V3PositionManager.INTERFACE.getSighash("decreaseLiquidity") ||
		...
		// encode v4 mint
		const v4AddParams = V4PositionManager.addCallParameters(
			options.outputPosition,
			options.v4AddLiquidityOptions,
		);
		// only modifyLiquidities can be called by the UniversalRouter
		const selector = v4AddParams.calldata.slice(0, 10);
		invariant(
			selector == V4PositionManager.INTERFACE.getSighash("modifyLiquidities"),
			"INVALID_V4_CALL: " + selector,
		);

		planner.addCommand(CommandType.V4_POSITION_MANAGER_CALL, [
			v4AddParams.calldata,
		]);
	...
	private static encodePlan(
		...
		const functionSignature = !!config.deadline
			? "execute(bytes,bytes[],uint256)"
			: "execute(bytes,bytes[])";
		const parameters = !!config.deadline
			? [commands, inputs, config.deadline]
			: [commands, inputs];
		const calldata = SwapRouter.INTERFACE.encodeFunctionData(
			functionSignature,
			parameters,
		);
