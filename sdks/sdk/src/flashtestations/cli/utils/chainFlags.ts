...
export function addChainFlags(command: Command): Command {
  ...
  return command
    .option('--chain <name>', `Chain to use (${availableChains}) [default: ${getDefaultChainSlug()}]`)
...
export function resolveChainConfig(options: ChainOptions): ResolvedChainConfig {
  ...
  if (options.chainId !== undefined) {
    ...
    if (!config) {
      const availableChains = getSupportedChainSlugs().join(', ')
      throw new ValidationError(`Unknown chain "${options.chain}". Available chains: ${availableChains}`)
    }
  ...
  if (!options.rpcUrl) {
    ...
    if (getRpcUrl(chainId) === '') {
      throw new ValidationError(
        `Chain ${chainId} requires a custom RPC URL. ` + `Use --rpc-url to specify a custom RPC URL for this chain.`
      )
