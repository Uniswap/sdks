...
export class RelayOrderTrade<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType> {
  ...
  private getInputAmount(): CurrencyAmount<TInput> {
    ...
    // assume single chain ids across all outputs for now
    const currencyIn = this._currenciesIn.find((currency) =>
      areCurrenciesEqual(currency, this.order.info.input.token, currency.chainId)
    )
