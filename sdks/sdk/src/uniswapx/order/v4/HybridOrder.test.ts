...
const COSIGNER = new Wallet('0x59c6995e998f97a5a0044976f7d75e7b7d6f4b6b55bdbb1c0cfd43a3d6ab1e31')
...
describe('CosignedHybridOrder', () => {
  ...
  function packPriceCurveElement(duration: number, scalingFactor: BigNumber): BigNumber {
  ...
  function buildOrder(overrides: Partial<CosignedHybridOrderInfo> = {}): CosignedHybridOrderInfo {
    ...
    const merged: CosignedHybridOrderInfo = {
      ...
      input: {
        ...baseOrder.input,
        ...(overrides.input ?? {}),
        maxAmount: BigNumber.from(overrides.input?.maxAmount ?? baseOrder.input.maxAmount),
      },
      outputs: (overrides.outputs ?? baseOrder.outputs).map((output) => ({
        ...output,
        minAmount: BigNumber.from(output.minAmount),
      })),
      auctionStartBlock: BigNumber.from(overrides.auctionStartBlock ?? baseOrder.auctionStartBlock),
      baselinePriorityFee: BigNumber.from(overrides.baselinePriorityFee ?? baseOrder.baselinePriorityFee),
      scalingFactor: BigNumber.from(overrides.scalingFactor ?? baseOrder.scalingFactor),
      priceCurve: (overrides.priceCurve ?? baseOrder.priceCurve).map((value: BigNumber) => BigNumber.from(value)),
      cosignerData: {
        auctionTargetBlock: BigNumber.from(
          overrides.cosignerData?.auctionTargetBlock ?? baseOrder.cosignerData.auctionTargetBlock
        ),
        supplementalPriceCurve: (
          overrides.cosignerData?.supplementalPriceCurve ?? baseOrder.cosignerData.supplementalPriceCurve
    ...
    if (merged.cosigner !== ZERO_ADDRESS && (overrides.cosignature === undefined || overrides.cosignature === '0x')) {
  ...
  it('serializes to resolver-prefixed bytes', () => {
    ...
    const [resolver, encoded] = abiCoder.decode(['address', 'bytes'], serialized)
    ...
    const manualEncoding = abiCoder.encode(HYBRID_ORDER_ABI, [
      [
        ...
        order.outputs.map((output) => [output.token, output.minAmount, output.recipient]),
        order.auctionStartBlock,
        order.baselinePriorityFee,
        order.scalingFactor,
        order.priceCurve,
        [order.cosignerData.auctionTargetBlock, order.cosignerData.supplementalPriceCurve],
        order.cosignature,
      ],
    ])
  ...
  it('round-trips via JSON serialization', () => {
    ...
    const revived = CosignedHybridOrder.fromJSON(json as CosignedHybridOrderInfoJSON, chainId, RESOLVER)
  ...
  it('computes cosigner digest and recovers signer', () => {
    const order = buildOrder()
    const hybrid = new CosignedHybridOrder(order, chainId, RESOLVER)
    const digest = hashHybridCosignerData(hashHybridOrder(order), order.cosignerData, chainId)
    expect(hybrid.cosignatureHash()).toEqual(digest)
    expect(ethers.utils.getAddress(hybrid.recoverCosigner())).toEqual(ethers.utils.getAddress(COSIGNER.address))
  })
  ...
  it('prefers cosigner auction target block for overrides', () => {
    ...
    const hybrid = new CosignedHybridOrder(order, chainId, RESOLVER)
    expect(hybrid.blockOverrides).toEqual({
      number: ethers.utils.hexStripZeros(order.cosignerData.auctionTargetBlock.toHexString()),
    })
  })
  ...
  it('resolves exact-in path when scalingFactor >= 1e18', () => {
    ...
    const expectedOutput = mulWadUp(order.outputs[0].minAmount, expectedMultiplier)
