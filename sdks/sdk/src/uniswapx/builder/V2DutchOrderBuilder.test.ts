...
describe('V2DutchOrderBuilder', () => {
  ...
  it('outputOverride smaller than output', () => {
    ...
    expect(() =>
      builder
        ...
        .build()
    ).toThrow('Invariant failed: outputOverride must be larger than or equal to original output')
  })
  ...
  it('Start time after deadline', () => {
    ...
    expect(() =>
      builder
        ...
        .outputOverrides([OUTPUT_START_AMOUNT.mul(102).div(100)])
        .build()
    ).toThrow(`Invariant failed: decayStartTime must be before or same as deadline: ${deadline + 100}`)
  })
  ...
  it('decayEndTime after deadline', () => {
    ...
    expect(() =>
      builder
        ...
        .outputOverrides([OUTPUT_START_AMOUNT.mul(102).div(100)])
        .build()
    ).toThrow(`Invariant failed: decayEndTime must be before or same as deadline: ${deadline + 100}`)
  })
  ...
  describe('partial order tests', () => {
    it('builds an unsigned partial order with default cosignerData values', () => {
      //TODO: partial orders don't have cosignerData...
      const deadline = Math.floor(Date.now() / 1000) + 1000
      const order = builder
        .cosigner(constants.AddressZero)
        .swapper(constants.AddressZero)
