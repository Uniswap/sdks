...
export class HybridOrderBuilder {
  ...
  auctionStartBlock(block: BigNumber | number): this {
    this.orderData.auctionStartBlock = typeof block === 'number' ? BigNumber.from(block) : block
    return this
  }

  baselinePriorityFee(fee: BigNumber | number): this {
    this.orderData.baselinePriorityFee = typeof fee === 'number' ? BigNumber.from(fee) : fee
    return this
  }
  ...
  auctionTargetBlock(block: BigNumber | number): this {
    if (!this.orderData.cosignerData) {
      ...
      this.orderData.cosignerData.auctionTargetBlock = typeof block === 'number' ? BigNumber.from(block) : block
    }
    return this
  }
  ...
  private checkCosignedInvariants(): void {
    invariant(this.orderData.cosignature !== undefined && this.orderData.cosignature !== '0x', 'cosignature not set')
