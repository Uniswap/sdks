...
describe('verify.ts', () => {
  describe('Test failure cases for verifyAsyncDns', () => {
    ...
    it('throws error if DNS resolution fails', async () => {
      ...
      await expect(verifyAsyncDns('data', 'signature', 'example.com', '1')).rejects.toThrow('DNS resolution failed')
    })

    it('throws error if no TXT records are found', async () => {
      mockQuery.mockResolvedValue({ answers: [] })

      await expect(verifyAsyncDns('data', 'signature', 'example.com', '1')).rejects.toThrow(
        'No TXT records found for host example.com'
      )
    })
  ...
  describe('TXT record parsing for verifyAsyncDns', () => {
    ...
    it('should handle single substring TXT records as string', async () => {
      ...
      await expect(verifyAsyncDns('data', 'signature', 'example.com', '1')).rejects.toThrow() // Will fail at crypto step, but parsing succeeded
    })
    ...
    it('sanitizes leading slashes and encodes TWIST path segments', async () => {
      ...
      global.fetch = vi.fn().mockResolvedValue({
        ...
        status: 200,
        headers: {
          get: (name: string) => (name === 'content-type' ? 'application/json' : '0'),
        },
        json: () =>
          Promise.resolve({
      ...
      expect(urlArg.href).toBe('https://example.com/api%20v1/%C6%99eys%3Fbad%23frag')
    })
    ...
    it('rejects when TWIST path exceeds maximum length', async () => {
      ...
      await expect(verifyAsyncDns('data', 'signature', 'example.com', '1')).rejects.toThrow('TWIST path too long')
  ...
  describe('verifyAsyncJson', () => {
    ...
    it('throws if URL is not HTTPS', async () => {
      ...
      await expect(verifyAsyncJson('data', 'signature', httpUrl, '1')).rejects.toThrow(
    ...
    it('throws if HTTP status is not ok', async () => {
      ...
      await expect(verifyAsyncJson('data', 'signature', httpsUrl, '1')).rejects.toThrow(
    ...
    it('throws if key id is not found', async () => {
      global.fetch = vi.fn().mockResolvedValue({
        ...
        headers: {
          get: (name: string) =>
            name === 'content-type' ? 'application/json' : name === 'content-length' ? '0' : null,
        },
        json: () => Promise.resolve({ publicKeys: [] }),
      })

      await expect(verifyAsyncJson('data', 'signature', httpsUrl, '1')).rejects.toThrow(
        'Public key with id 1 not found'
      )
    })
    ...
    it('throws if algorithm is unsupported', async () => {
      ...
      await expect(verifyAsyncJson('data', 'signature', httpsUrl, '1')).rejects.toThrow(
        'Algorithm is not supported: UNSUPPORTED'
      )
    })
    ...
    it('returns true when signature verifies for RS256', async () => {
      ...
      await expect(verifyAsyncJson(data, signatureHex, httpsUrl, '1')).resolves.toBe(true)
    })
    ...
    it('accepts Content-Type with charset parameter', async () => {
      ...
      global.fetch = vi.fn().mockResolvedValue({
        ...
        status: 200,
        headers: {
          get: (name: string) => (name === 'content-type' ? 'application/json; charset=utf-8' : '0'),
        },
        json: () =>
          Promise.resolve({
            publicKeys: [{ id: '1', alg: 'RS256', publicKey: spkiHex }],
    ...
    it('throws if manifest exceeds maximum size', async () => {
      global.fetch = vi.fn().mockResolvedValue({
        ...
        headers: {
          get: (name: string) =>
            name === 'content-type' ? 'application/json' : name === 'content-length' ? String(64 * 1024 + 1) : null,
        },
        json: () => Promise.resolve({ publicKeys: [] }),
      })

      await expect(verifyAsyncJson('data', 'signature', httpsUrl, '1')).rejects.toThrow('Manifest too large')
    })
  ...
  describe('verify', () => {
    ...
    describe('returns true for correct public key', () => {
      it('is successful with ES256 (ECDSA P-256)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'ES256')).toBe(true)
      })
      it('is successful with EdDSA (Ed25519)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'EdDSA')).toBe(true)
      })
      it('is successful with ES384 (ECDSA P-384)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'ES384')).toBe(true)
      })
      it('is successful with ES512 (ECDSA P-521)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'ES512')).toBe(true)
      })
      it('is successful with RS384 (RSASSA-PKCS1-v1_5)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'RS384')).toBe(true)
      })
      it('is successful with RS512 (RSASSA-PKCS1-v1_5)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'RS512')).toBe(true)
      })
      it('is successful with PS384 (RSA-PSS)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'PS384')).toBe(true)
      })
      it('is successful with PS512 (RSA-PSS)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'PS512')).toBe(true)
      })
      it('is successful with RS256 (RSASSA-PKCS1-v1_5)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'RS256')).toBe(true)
      })
      it('is successful with PS256 (RSA-PSS)', async () => {
        ...
        expect(await verify(data, signatureString, publicKey, 'PS256')).toBe(true)
      })
    })
    ...
    it('verifies object payload using canonical JSON (RS256)', async () => {
      ...
      await expect(verify(canonical, signatureHex, rsaSSAKeyPair.publicKey, 'RS256')).resolves.toBe(true)
    })
    ...
    it('returns false for incorrect public key', async () => {
      ...
      expect(await verify(data, signatureString, publicKey2, 'RS256')).toBe(false)
    })
    ...
    it('returns false for malformed ECDSA signature length (ES256)', async () => {
      ...
      await expect(verify(data, invalidLengthHex, publicKey, 'ES256')).resolves.toBe(false)
    })
  })
})
