...
import { SigningAlgorithmConfig, SIGNING_ALGORITHM_CONFIG, SIGNING_ALGORITHM_IMPORT_PARAMS } from './algorithms.js'
import {
...
export async function verifyAsyncJson(calldata: string, signature: string, url: URL, id: string): Promise<boolean> {
  ...
  if (!Object.prototype.hasOwnProperty.call(SIGNING_ALGORITHM_IMPORT_PARAMS, publicKey.alg)) {
    throw new Error(ERROR_ALGORITHM_NOT_SUPPORTED(publicKey.alg))
  }
  const algorithmKey = publicKey.alg as keyof typeof SIGNING_ALGORITHM_IMPORT_PARAMS
...
export async function verify(
  ...
  const verifyParams = algConfig as unknown as Algorithm | EcdsaParams | RsaPssParams
