...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant(JSBI2.lessThanOrEqual(this.quotient, MaxUint256), "AMOUNT");
...
var MAX_SAFE_INTEGER = JSBI2.BigInt(Number.MAX_SAFE_INTEGER);
var ZERO = JSBI2.BigInt(0);
var ONE = JSBI2.BigInt(1);
var TWO = JSBI2.BigInt(2);
function sqrt(value) {
  invariant(JSBI2.greaterThanOrEqual(value, ZERO), "NEGATIVE");
  if (JSBI2.lessThan(value, MAX_SAFE_INTEGER)) {
    return JSBI2.BigInt(Math.floor(Math.sqrt(JSBI2.toNumber(value))));
  }
  let z;
  let x;
  z = value;
  x = JSBI2.add(JSBI2.divide(value, TWO), ONE);
  while (JSBI2.lessThan(x, z)) {
    z = x;
    x = JSBI2.divide(JSBI2.add(JSBI2.divide(value, x), x), TWO);
  }
  return z;
}
