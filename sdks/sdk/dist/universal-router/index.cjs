...
var UniversalRouter = require('@uniswap/universal-router/artifacts/contracts/UniversalRouter.sol/UniversalRouter.json');
...
var UniversalRouter__default = /*#__PURE__*/_interopDefault(UniversalRouter);
...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant18__default.default(JSBI13__default.default.lessThanOrEqual(this.quotient, MaxUint256), "AMOUNT");
...
function sqrt(value) {
  invariant18__default.default(JSBI13__default.default.greaterThanOrEqual(value, ZERO), "NEGATIVE");
...
var SqrtPriceMath = class {
  ...
  static getNextSqrtPriceFromInput(sqrtPX96, liquidity, amountIn, zeroForOne) {
    invariant18__default.default(JSBI13__default.default.greaterThan(sqrtPX96, ZERO2));
    invariant18__default.default(JSBI13__default.default.greaterThan(liquidity, ZERO2));
  ...
  static getNextSqrtPriceFromAmount0RoundingUp(sqrtPX96, liquidity, amount, add) {
    ...
    if (add) {
      ...
      let product = multiplyIn256(amount, sqrtPX96);
      invariant18__default.default(JSBI13__default.default.equal(JSBI13__default.default.divide(product, amount), sqrtPX96));
      invariant18__default.default(JSBI13__default.default.greaterThan(numerator1, product));
  ...
  static getNextSqrtPriceFromAmount1RoundingDown(sqrtPX96, liquidity, amount, add) {
    if (add) {
      ...
      invariant18__default.default(JSBI13__default.default.greaterThan(sqrtPX96, quotient));
...
function mostSignificantBit(x) {
  invariant18__default.default(JSBI13__default.default.greaterThan(x, ZERO2), "ZERO");
  invariant18__default.default(JSBI13__default.default.lessThanOrEqual(x, MaxUint256), "MAX");
  let msb = 0;
...
async function v3Swap(fee, sqrtRatioX96, tickCurrent, liquidity, tickSpacing, tickDataProvider, zeroForOne, amountSpecified, sqrtPriceLimitX96) {
  ...
  if (zeroForOne) {
    invariant18__default.default(JSBI13__default.default.greaterThan(sqrtPriceLimitX96, TickMath.MIN_SQRT_RATIO), "RATIO_MIN");
    invariant18__default.default(JSBI13__default.default.lessThan(sqrtPriceLimitX96, sqrtRatioX96), "RATIO_CURRENT");
  } else {
    invariant18__default.default(JSBI13__default.default.lessThan(sqrtPriceLimitX96, TickMath.MAX_SQRT_RATIO), "RATIO_MAX");
    invariant18__default.default(JSBI13__default.default.greaterThan(sqrtPriceLimitX96, sqrtRatioX96), "RATIO_CURRENT");
  }
...
var MaxUint128 = toHex2(JSBI13__default.default.subtract(JSBI13__default.default.exponentiate(JSBI13__default.default.BigInt(2), JSBI13__default.default.BigInt(128)), JSBI13__default.default.BigInt(1)));
...
var _NonfungiblePositionManager = class _NonfungiblePositionManager {
  ...
  static addCallParameters(position, options) {
    invariant18__default.default(JSBI13__default.default.greaterThan(position.liquidity, ZERO2), "ZERO_LIQUIDITY");
    const calldatas = [];
  ...
  static removeCallParameters(position, options) {
    ...
    invariant18__default.default(JSBI13__default.default.greaterThan(partialPosition.liquidity, ZERO2), "ZERO_LIQUIDITY");
...
var ADDRESS_ZERO2 = ethers.constants.AddressZero;
var NEGATIVE_ONE2 = JSBI13__default.default.BigInt(-1);
var ZERO3 = JSBI13__default.default.BigInt(0);
var ONE3 = JSBI13__default.default.BigInt(1);
JSBI13__default.default.exponentiate(JSBI13__default.default.BigInt(10), JSBI13__default.default.BigInt(18));
var EMPTY_BYTES = "0x";
var Q962 = JSBI13__default.default.exponentiate(JSBI13__default.default.BigInt(2), JSBI13__default.default.BigInt(96));
var Q1922 = JSBI13__default.default.exponentiate(Q962, JSBI13__default.default.BigInt(2));
...
var Pool2 = class _Pool {
  ...
  async getOutputAmount(inputAmount, sqrtPriceLimitX96) {
    ...
    return [
      CurrencyAmount.fromRawAmount(outputCurrency, JSBI13__default.default.multiply(outputAmount, NEGATIVE_ONE2)),
      new _Pool(
        this.currency0,
        this.currency1,
        this.fee,
  ...
  async getInputAmount(outputAmount, sqrtPriceLimitX96) {
    ...
    const {
      ...
      liquidity,
      tickCurrent
    } = await this.swap(zeroForOne, JSBI13__default.default.multiply(outputAmount.quotient, NEGATIVE_ONE2), sqrtPriceLimitX96);
...
var V4Planner = class {
  ...
  addTrade(trade, slippageTolerance) {
    ...
    if (exactOutput) invariant18__default.default(!!slippageTolerance, "ExactOut requires slippageTolerance");
    invariant18__default.default(trade.swaps.length === 1, "Only accepts Trades with 1 swap (must break swaps into individual trades)");
...
var V4BaseActionsParser = class _V4BaseActionsParser {
  static parseCalldata(calldata) {
    const [actions, inputs] = ethers.ethers.utils.defaultAbiCoder.decode(["bytes", "bytes[]"], calldata);
    ...
    return {
      actions: actionTypes.map((actionType, i) => {
        ...
        const rawParams = ethers.ethers.utils.defaultAbiCoder.decode(
...
_Multicall2.INTERFACE = new abi.Interface(IMulticall__default.default.abi);
...
function shouldCreatePool(options) {
  if (options.createPool) {
    invariant18__default.default(options.sqrtPriceX96 !== void 0, NO_SQRT_PRICE);
    return true;
  }
  return false;
}
var _V4PositionManager = class _V4PositionManager {
  ...
  static addCallParameters(position, options) {
    invariant18__default.default(JSBI13__default.default.greaterThan(position.liquidity, ZERO3), ZERO_LIQUIDITY);
    const calldataList = [];
  ...
  static removeCallParameters(position, options) {
    ...
    if (options.burnToken) {
      invariant18__default.default(options.liquidityPercentage.equalTo(ONE3), CANNOT_BURN);
      if (options.permit) {
        calldataList.push(
      ...
      invariant18__default.default(JSBI13__default.default.greaterThan(partialPosition.liquidity, ZERO3), ZERO_LIQUIDITY);
      const { amount0: amount0Min, amount1: amount1Min } = partialPosition.burnAmountsWithSlippage(
        options.slippageTolerance
      );
      planner.addDecrease(
...
_V4PositionManager.INTERFACE = new abi.Interface(positionManagerAbi);
...
var MINIMUM_LIQUIDITY = JSBI13__default.default.BigInt(1e3);
var ZERO4 = JSBI13__default.default.BigInt(0);
var ONE4 = JSBI13__default.default.BigInt(1);
var FIVE = JSBI13__default.default.BigInt(5);
var _997 = JSBI13__default.default.BigInt(997);
var _1000 = JSBI13__default.default.BigInt(1e3);
var BASIS_POINTS = JSBI13__default.default.BigInt(1e4);
...
var Pair = class _Pair {
  ...
  getOutputAmount(inputAmount, calculateFotFees = true) {
    invariant18__default.default(this.involvesToken(inputAmount.currency), "TOKEN");
    if (JSBI13__default.default.equal(this.reserve0.quotient, ZERO4) || JSBI13__default.default.equal(this.reserve1.quotient, ZERO4)) {
    ...
    const inputAmountWithFeeAndAfterTax = JSBI13__default.default.multiply(inputAmountAfterTax.quotient, _997);
    const numerator = JSBI13__default.default.multiply(inputAmountWithFeeAndAfterTax, outputReserve.quotient);
    const denominator = JSBI13__default.default.add(JSBI13__default.default.multiply(inputReserve.quotient, _1000), inputAmountWithFeeAndAfterTax);
    const outputAmount = CurrencyAmount.fromRawAmount(
      inputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
      JSBI13__default.default.divide(numerator, denominator)
      // JSBI.divide will round down by itself, which is desired
    );
    if (JSBI13__default.default.equal(outputAmount.quotient, ZERO4)) {
    ...
    if (JSBI13__default.default.equal(outputAmountAfterTax.quotient, ZERO4)) {
      throw new InsufficientInputAmountError();
    }
    return [
  ...
  getInputAmount(outputAmount, calculateFotFees = true) {
    ...
    const outputAmountBeforeTax = percentAfterBuyFees.greaterThan(ZERO_PERCENT) ? CurrencyAmount.fromRawAmount(
      outputAmount.currency,
      JSBI13__default.default.add(outputAmount.divide(percentAfterBuyFees).quotient, ONE4)
      // add 1 for rounding up
    ) : outputAmount;
    if (JSBI13__default.default.equal(this.reserve0.quotient, ZERO4) || JSBI13__default.default.equal(this.reserve1.quotient, ZERO4) || JSBI13__default.default.greaterThanOrEqual(outputAmount.quotient, this.reserveOf(outputAmount.currency).quotient) || JSBI13__default.default.greaterThanOrEqual(outputAmountBeforeTax.quotient, this.reserveOf(outputAmount.currency).quotient)) {
      throw new InsufficientReservesError();
    }
    const outputReserve = this.reserveOf(outputAmount.currency);
    const inputReserve = this.reserveOf(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0);
    const numerator = JSBI13__default.default.multiply(JSBI13__default.default.multiply(inputReserve.quotient, outputAmountBeforeTax.quotient), _1000);
    const denominator = JSBI13__default.default.multiply(JSBI13__default.default.subtract(outputReserve.quotient, outputAmountBeforeTax.quotient), _997);
    const inputAmount = CurrencyAmount.fromRawAmount(
      outputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
      JSBI13__default.default.add(JSBI13__default.default.divide(numerator, denominator), ONE4)
    ...
    const inputAmountBeforeTax = percentAfterSellFees.greaterThan(ZERO_PERCENT) ? CurrencyAmount.fromRawAmount(
      inputAmount.currency,
      JSBI13__default.default.add(inputAmount.divide(percentAfterSellFees).quotient, ONE4)
      // add 1 for rounding up
    ) : inputAmount;
  ...
  getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    ...
    let liquidity;
    if (JSBI13__default.default.equal(totalSupply.quotient, ZERO4)) {
      liquidity = JSBI13__default.default.subtract(
        sqrt(JSBI13__default.default.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)),
        MINIMUM_LIQUIDITY
      );
    } else {
      const amount0 = JSBI13__default.default.divide(JSBI13__default.default.multiply(tokenAmounts[0].quotient, totalSupply.quotient), this.reserve0.quotient);
      const amount1 = JSBI13__default.default.divide(JSBI13__default.default.multiply(tokenAmounts[1].quotient, totalSupply.quotient), this.reserve1.quotient);
      liquidity = JSBI13__default.default.lessThanOrEqual(amount0, amount1) ? amount0 : amount1;
    }
    if (!JSBI13__default.default.greaterThan(liquidity, ZERO4)) {
      throw new InsufficientInputAmountError();
    }
  ...
  getLiquidityValue(token, totalSupply, liquidity, feeOn = false, kLast) {
    ...
    invariant18__default.default(JSBI13__default.default.lessThanOrEqual(liquidity.quotient, totalSupply.quotient), "LIQUIDITY");
    let totalSupplyAdjusted;
    if (!feeOn) {
      totalSupplyAdjusted = totalSupply;
    } else {
      invariant18__default.default(!!kLast, "K_LAST");
      const kLastParsed = JSBI13__default.default.BigInt(kLast);
      if (!JSBI13__default.default.equal(kLastParsed, ZERO4)) {
        const rootK = sqrt(JSBI13__default.default.multiply(this.reserve0.quotient, this.reserve1.quotient));
        const rootKLast = sqrt(kLastParsed);
        if (JSBI13__default.default.greaterThan(rootK, rootKLast)) {
          const numerator = JSBI13__default.default.multiply(totalSupply.quotient, JSBI13__default.default.subtract(rootK, rootKLast));
          const denominator = JSBI13__default.default.add(JSBI13__default.default.multiply(rootK, FIVE), rootKLast);
          const feeLiquidity = JSBI13__default.default.divide(numerator, denominator);
    ...
    return CurrencyAmount.fromRawAmount(
      token,
      JSBI13__default.default.divide(JSBI13__default.default.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient)
    );
  }
  derivePercentAfterSellFees(inputAmount) {
    ...
    if (sellFeeBips?.gt(BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new Percent(JSBI13__default.default.BigInt(sellFeeBips)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  }
  derivePercentAfterBuyFees(outputAmount) {
    ...
    if (buyFeeBps?.gt(BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new Percent(JSBI13__default.default.BigInt(buyFeeBps)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  }
...
var ZERO5 = JSBI13__default.default.BigInt(0);
var ONE5 = JSBI13__default.default.BigInt(1);
...
var CONTRACT_BALANCE = ethers.BigNumber.from(2).pow(255);
...
var UniswapTrade = class {
  ...
  encode(planner, _config) {
    ...
    let minimumAmountOut = ethers.BigNumber.from(
...
function encodeInputTokenOptions(planner, options) {
  if (!!options.permit2TransferFrom && !!options.permit2Permit)
    invariant18__default.default(options.permit2TransferFrom.token === options.permit2Permit.details.token, `inconsistent token`);
...
var _SwapRouter = class _SwapRouter {
  static swapCallParameters(trades, options) {
    ...
    invariant18__default.default(!(inputCurrency.isNative && !!options.inputTokenPermit), "NATIVE_INPUT_PERMIT");
    if (options.inputTokenPermit) {
      encodePermit(planner, options.inputTokenPermit);
    }
    const nativeCurrencyValue = inputCurrency.isNative ? ethers.BigNumber.from(trade.trade.maximumAmountIn(options.slippageTolerance).quotient.toString()) : ethers.BigNumber.from(0);
    trade.encode(planner, { allowRevert: false });
    return _SwapRouter.encodePlan(planner, nativeCurrencyValue, {
      deadline: options.deadlineOrPreviousBlockhash ? ethers.BigNumber.from(options.deadlineOrPreviousBlockhash) : void 0
    });
  }
  /**
  ...
  static migrateV3ToV4CallParameters(options, positionManagerOverride) {
    ...
    if (v4Pool.currency0.isNative) {
      ...
      invariant18__default.default(v3Token0 === v4Pool.token0, "TOKEN0_MISMATCH");
      invariant18__default.default(v3Token1 === v4Pool.token1, "TOKEN1_MISMATCH");
    }
    invariant18__default.default(
      options.v3RemoveLiquidityOptions.liquidityPercentage.equalTo(new Percent(100, 100)),
      "FULL_REMOVAL_REQUIRED"
    );
    invariant18__default.default(options.v3RemoveLiquidityOptions.burnToken == true, "BURN_TOKEN_REQUIRED");
    invariant18__default.default(
      options.v3RemoveLiquidityOptions.collectOptions.recipient === v4PositionManagerAddress,
      "RECIPIENT_NOT_POSITION_MANAGER"
    );
    invariant18__default.default(isMint3(options.v4AddLiquidityOptions), "MINT_REQUIRED");
    invariant18__default.default(options.v4AddLiquidityOptions.migrate, "MIGRATE_REQUIRED");
    const planner = new RoutePlanner();
    ...
    if (options.v3RemoveLiquidityOptions.permit) {
      ...
      invariant18__default.default(universalRouterAddress == options.v3RemoveLiquidityOptions.permit.spender, "INVALID_SPENDER");
    ...
    invariant18__default.default(selector == V4PositionManager.INTERFACE.getSighash("modifyLiquidities"), "INVALID_V4_CALL: " + selector);
    planner.addCommand(20 /* V4_POSITION_MANAGER_CALL */, [v4AddParams.calldata]);
    return _SwapRouter.encodePlan(planner, ethers.BigNumber.from(0), {
...
_SwapRouter.INTERFACE = new abi.Interface(UniversalRouter__default.default.abi);
...
var UnwrapWETH = class {
  constructor(amount, chainId, permit2) {
    ...
    if (!!permit2) {
      ...
      invariant18__default.default(permit2.details.amount >= amount, `Did not permit enough WETH for unwrapWETH transaction`);
...
var _RouterTradeAdapter = class _RouterTradeAdapter {
  ...
  static toToken(token) {
    ...
    return new Token(
      ...
      void 0,
      false,
      buyFeeBps ? ethers.BigNumber.from(buyFeeBps) : void 0,
      sellFeeBps ? ethers.BigNumber.from(sellFeeBps) : void 0
    );
  }
...
_CommandParser.INTERFACE = new abi.Interface(UniversalRouter__default.default.abi);
...
function parseV3PathExactIn(path) {
  ...
  let tokenIn = ethers.ethers.utils.getAddress(strippedPath.substring(0, 40));
  let loc = 40;
  const res = [];
  while (loc < strippedPath.length) {
    ...
    const tokenOut = ethers.ethers.utils.getAddress(feeAndTokenOut.substring(6, 46));
    res.push({
      tokenIn,
      tokenOut,
      fee
...
function parseV3PathExactOut(path) {
  const strippedPath = path.replace("0x", "");
  let tokenIn = ethers.ethers.utils.getAddress(strippedPath.substring(strippedPath.length - 40));
  let loc = strippedPath.length - 86;
  const res = [];
  while (loc >= 0) {
    const feeAndTokenOut = strippedPath.substring(loc, loc + 46);
    const tokenOut = ethers.ethers.utils.getAddress(feeAndTokenOut.substring(0, 40));
...
exports.RouterActionType = RouterActionType;
exports.RouterTradeAdapter = RouterTradeAdapter;
exports.Subparser = Subparser2;
...
exports.UniversalRouterVersion = UniversalRouterVersion;
