...
import invariant16 from 'tiny-invariant';
...
var Fraction = class _Fraction {
  ...
  toSignificant(significantDigits, format = { groupSeparator: "" }, rounding = 1 /* ROUND_HALF_UP */) {
    invariant16(Number.isInteger(significantDigits), `${significantDigits} is not an integer.`);
    invariant16(significantDigits > 0, `${significantDigits} is not positive.`);
    Decimal.set({ precision: significantDigits + 1, rounding: toSignificantRounding[rounding] });
    const quotient = new Decimal(this.numerator.toString()).div(this.denominator.toString()).toSignificantDigits(significantDigits);
    return quotient.toFormat(quotient.decimalPlaces(), format);
  }
  toFixed(decimalPlaces, format = { groupSeparator: "" }, rounding = 1 /* ROUND_HALF_UP */) {
    invariant16(Number.isInteger(decimalPlaces), `${decimalPlaces} is not an integer.`);
    invariant16(decimalPlaces >= 0, `${decimalPlaces} is negative.`);
...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant16(JSBI13.lessThanOrEqual(this.quotient, MaxUint256), "AMOUNT");
    this.currency = currency;
    this.decimalScale = JSBI13.exponentiate(JSBI13.BigInt(10), JSBI13.BigInt(currency.decimals));
  }
  add(other) {
    invariant16(this.currency.equals(other.currency), "CURRENCY");
  ...
  toFixed(decimalPlaces = this.currency.decimals, format, rounding = 0 /* ROUND_DOWN */) {
    invariant16(decimalPlaces <= this.currency.decimals, "DECIMALS");
...
var Price = class _Price extends Fraction {
   ...
   */
  multiply(other) {
    invariant16(this.quoteCurrency.equals(other.baseCurrency), "TOKEN");
  ...
  quote(currencyAmount) {
    invariant16(currencyAmount.currency.equals(this.baseCurrency), "TOKEN");
...
function sortedInsert(items, add, maxSize, comparator) {
  invariant16(maxSize > 0, "MAX_SIZE_ZERO");
  invariant16(items.length <= maxSize, "ITEMS_SIZE");
  if (items.length === 0) {
    items.push(add);
    return null;
  } else {
...
function sqrt(value) {
  invariant16(JSBI13.greaterThanOrEqual(value, ZERO), "NEGATIVE");
...
var SqrtPriceMath = class {
  ...
  static getNextSqrtPriceFromInput(sqrtPX96, liquidity, amountIn, zeroForOne) {
    invariant16(JSBI13.greaterThan(sqrtPX96, ZERO2));
    invariant16(JSBI13.greaterThan(liquidity, ZERO2));
  ...
  static getNextSqrtPriceFromAmount0RoundingUp(sqrtPX96, liquidity, amount, add) {
    ...
    if (add) {
      ...
      let product = multiplyIn256(amount, sqrtPX96);
      invariant16(JSBI13.equal(JSBI13.divide(product, amount), sqrtPX96));
      invariant16(JSBI13.greaterThan(numerator1, product));
  ...
  static getNextSqrtPriceFromAmount1RoundingDown(sqrtPX96, liquidity, amount, add) {
    if (add) {
      ...
      invariant16(JSBI13.greaterThan(sqrtPX96, quotient));
...
function mostSignificantBit(x) {
  invariant16(JSBI13.greaterThan(x, ZERO2), "ZERO");
  invariant16(JSBI13.lessThanOrEqual(x, MaxUint256), "MAX");
  let msb = 0;
...
var _TickMath = class _TickMath {
  ...
  static getSqrtRatioAtTick(tick) {
    invariant16(tick >= _TickMath.MIN_TICK && tick <= _TickMath.MAX_TICK && Number.isInteger(tick), "TICK");
...
async function v3Swap(fee, sqrtRatioX96, tickCurrent, liquidity, tickSpacing, tickDataProvider, zeroForOne, amountSpecified, sqrtPriceLimitX96) {
  ...
  if (zeroForOne) {
    invariant16(JSBI13.greaterThan(sqrtPriceLimitX96, TickMath.MIN_SQRT_RATIO), "RATIO_MIN");
    invariant16(JSBI13.lessThan(sqrtPriceLimitX96, sqrtRatioX96), "RATIO_CURRENT");
  } else {
    invariant16(JSBI13.lessThan(sqrtPriceLimitX96, TickMath.MAX_SQRT_RATIO), "RATIO_MAX");
    invariant16(JSBI13.greaterThan(sqrtPriceLimitX96, sqrtRatioX96), "RATIO_CURRENT");
  }
...
var TickList = class _TickList {
  ...
  static validateList(ticks, tickSpacing) {
    invariant16(tickSpacing > 0, "TICK_SPACING_NONZERO");
    invariant16(
    ...
    invariant16(isSorted(ticks, tickComparator), "SORTED");
  }
  static isBelowSmallest(ticks, tick) {
    invariant16(ticks.length > 0, "LENGTH");
  ...
  static getTick(ticks, index) {
    ...
    invariant16(tick.index === index, "NOT_CONTAINED");
    return tick;
  }
  /**
  ...
  static binarySearch(ticks, tick) {
    invariant16(!this.isBelowSmallest(ticks, tick), "BELOW_SMALLEST");
    let l = 0;
  ...
  static nextInitializedTick(ticks, tick, lte) {
    if (lte) {
      invariant16(!_TickList.isBelowSmallest(ticks, tick), "BELOW_SMALLEST");
      ...
      invariant16(!this.isAtOrAboveLargest(ticks, tick), "AT_OR_ABOVE_LARGEST");
...
function nearestUsableTick(tick, tickSpacing) {
  invariant16(Number.isInteger(tick) && Number.isInteger(tickSpacing), "INTEGERS");
  invariant16(tickSpacing > 0, "TICK_SPACING");
  invariant16(tick >= TickMath.MIN_TICK && tick <= TickMath.MAX_TICK, "TICK_BOUND");
  const rounded = Math.round(tick / tickSpacing) * tickSpacing;
  if (rounded < TickMath.MIN_TICK) return rounded + tickSpacing;
  else if (rounded > TickMath.MAX_TICK) return rounded - tickSpacing;
  else return rounded;
}
var Q128 = JSBI13.exponentiate(JSBI13.BigInt(2), JSBI13.BigInt(128));
var PositionLibrary = class {
  ...
  static getTokensOwed(feeGrowthInside0LastX128, feeGrowthInside1LastX128, liquidity, feeGrowthInside0X128, feeGrowthInside1X128) {
    const tokensOwed0 = JSBI13.divide(
      JSBI13.multiply(subIn256(feeGrowthInside0X128, feeGrowthInside0LastX128), liquidity),
      Q128
    );
    const tokensOwed1 = JSBI13.divide(
      JSBI13.multiply(subIn256(feeGrowthInside1X128, feeGrowthInside1LastX128), liquidity),
      Q128
    );
...
var Q256 = JSBI13.exponentiate(JSBI13.BigInt(2), JSBI13.BigInt(256));
function subIn256(x, y) {
  const difference = JSBI13.subtract(x, y);
  if (JSBI13.lessThan(difference, ZERO2)) {
    return JSBI13.add(Q256, difference);
  } else {
    return difference;
  }
}
...
var Tick = class {
  constructor({ index, liquidityGross, liquidityNet }) {
    invariant16(index >= TickMath.MIN_TICK && index <= TickMath.MAX_TICK, "TICK");
    this.index = index;
...
var Pool = class _Pool {
  ...
  constructor(tokenA, tokenB, fee, sqrtRatioX96, liquidity, tickCurrent, ticks = NO_TICK_DATA_PROVIDER_DEFAULT) {
    invariant16(Number.isInteger(fee) && fee < 1e6, "FEE");
   ...
   */
  priceOf(token) {
    invariant16(this.involvesToken(token), "TOKEN");
  ...
  async getOutputAmount(inputAmount, sqrtPriceLimitX96) {
    invariant16(this.involvesToken(inputAmount.currency), "TOKEN");
  ...
  async getInputAmount(outputAmount, sqrtPriceLimitX96) {
    invariant16(outputAmount.currency.isToken && this.involvesToken(outputAmount.currency), "TOKEN");
...
var Position = class _Position {
  ...
  constructor({ pool, liquidity, tickLower, tickUpper }) {
    ...
    invariant16(tickLower < tickUpper, "TICK_ORDER");
    invariant16(tickLower >= TickMath.MIN_TICK && tickLower % pool.tickSpacing === 0, "TICK_LOWER");
    invariant16(tickUpper <= TickMath.MAX_TICK && tickUpper % pool.tickSpacing === 0, "TICK_UPPER");
    this.pool = pool;
...
var Route = class {
  ...
  constructor(pools, input, output) {
    this._midPrice = null;
    invariant16(pools.length > 0, "POOLS");
    const chainId = pools[0].chainId;
    const allOnSameChain = pools.every((pool) => pool.chainId === chainId);
    invariant16(allOnSameChain, "CHAIN_IDS");
    const wrappedInput = input.wrapped;
    invariant16(pools[0].involvesToken(wrappedInput), "INPUT");
    invariant16(pools[pools.length - 1].involvesToken(output.wrapped), "OUTPUT");
    const tokenPath = [wrappedInput];
    for (const [i, pool] of pools.entries()) {
      const currentInputToken = tokenPath[i];
      invariant16(currentInputToken.equals(pool.token0) || currentInputToken.equals(pool.token1), "PATH");
...
function tradeComparator(a, b) {
  invariant16(a.inputAmount.currency.equals(b.inputAmount.currency), "INPUT_CURRENCY");
  invariant16(a.outputAmount.currency.equals(b.outputAmount.currency), "OUTPUT_CURRENCY");
...
var Trade = class _Trade {
   ...
   */
  get route() {
    invariant16(this.swaps.length === 1, "MULTIPLE_ROUTES");
  ...
  static async fromRoute(route, amount, tradeType) {
    ...
    if (tradeType === 0 /* EXACT_INPUT */) {
      invariant16(amount.currency.equals(route.input), "INPUT");
      ...
      invariant16(amount.currency.equals(route.output), "OUTPUT");
  ...
  constructor({
    ...
    invariant16(numPools === poolAddressSet.size, "POOLS_DUPLICATED");
    this.swaps = routes;
  ...
  minimumAmountOut(slippageTolerance, amountOut = this.outputAmount) {
    invariant16(!slippageTolerance.lessThan(ZERO2), "SLIPPAGE_TOLERANCE");
  ...
  static async bestTradeExactIn(pools, currencyAmountIn, currencyOut, { maxNumResults = 3, maxHops = 3 } = {}, currentPools = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
    invariant16(pools.length > 0, "POOLS");
    invariant16(maxHops > 0, "MAX_HOPS");
    invariant16(currencyAmountIn === nextAmountIn || currentPools.length > 0, "INVALID_RECURSION");
  ...
  static async bestTradeExactOut(pools, currencyIn, currencyAmountOut, { maxNumResults = 3, maxHops = 3 } = {}, currentPools = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
    invariant16(pools.length > 0, "POOLS");
    invariant16(maxHops > 0, "MAX_HOPS");
    invariant16(currencyAmountOut === nextAmountOut || currentPools.length > 0, "INVALID_RECURSION");
...
var MaxUint128 = toHex(JSBI13.subtract(JSBI13.exponentiate(JSBI13.BigInt(2), JSBI13.BigInt(128)), JSBI13.BigInt(1)));
...
var _NonfungiblePositionManager = class _NonfungiblePositionManager {
  ...
  static addCallParameters(position, options) {
    invariant16(JSBI13.greaterThan(position.liquidity, ZERO2), "ZERO_LIQUIDITY");
    const calldatas = [];
    ...
    if (options.useNative) {
      ...
      invariant16(position.pool.token0.equals(wrapped) || position.pool.token1.equals(wrapped), "NO_WETH");
  ...
  static removeCallParameters(position, options) {
    ...
    invariant16(JSBI13.greaterThan(partialPosition.liquidity, ZERO2), "ZERO_LIQUIDITY");
    ...
    if (options.liquidityPercentage.equalTo(ONE2)) {
      ...
      invariant16(options.burnToken !== true, "CANNOT_BURN");
    }
    return {
      calldata: Multicall.encodeMulticall(calldatas),
      value: toHex(0)
...
var SwapQuoter = class {
  ...
  static quoteCallParameters(route, amount, tradeType, options = {}) {
    ...
    if (singleHop) {
      ...
      invariant16(options?.sqrtPriceLimitX96 === void 0, "MULTIHOP_PRICE_LIMIT");
...
_Staker.INTERFACE = new Interface(IUniswapV3Staker.abi);
...
var _SwapRouter = class _SwapRouter {
  ...
  static swapCallParameters(trades, options) {
    ...
    if (options.inputTokenPermit) {
      invariant16(sampleTrade.inputAmount.currency.isToken, "NON_TOKEN_PERMIT");
...
_SwapRouter.INTERFACE = new Interface(ISwapRouter.abi);
