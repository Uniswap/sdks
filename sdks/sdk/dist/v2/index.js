...
var MaxUint256 = JSBI8.BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
var RoundingMode = {
  RoundDown: 0,
  RoundHalfUp: 1,
  RoundUp: 3
...
var Fraction = class _Fraction {
  constructor(numerator, denominator = JSBI8.BigInt(1)) {
    this.numerator = JSBI8.BigInt(numerator);
    this.denominator = JSBI8.BigInt(denominator);
  }
  static tryParseFraction(fractionish) {
    if (fractionish instanceof JSBI8 || typeof fractionish === "number" || typeof fractionish === "string")
  ...
  get quotient() {
    return JSBI8.divide(this.numerator, this.denominator);
  }
  // remainder after floor division
  get remainder() {
    return new _Fraction(JSBI8.remainder(this.numerator, this.denominator), this.denominator);
  }
  invert() {
  ...
  add(other) {
    ...
    if (JSBI8.equal(this.denominator, otherParsed.denominator)) {
      return new _Fraction(JSBI8.add(this.numerator, otherParsed.numerator), this.denominator);
    }
    return new _Fraction(
      JSBI8.add(
        JSBI8.multiply(this.numerator, otherParsed.denominator),
        JSBI8.multiply(otherParsed.numerator, this.denominator)
      ),
      JSBI8.multiply(this.denominator, otherParsed.denominator)
    );
  }
  subtract(other) {
    const otherParsed = _Fraction.tryParseFraction(other);
    if (JSBI8.equal(this.denominator, otherParsed.denominator)) {
      return new _Fraction(JSBI8.subtract(this.numerator, otherParsed.numerator), this.denominator);
    }
    return new _Fraction(
      JSBI8.subtract(
        JSBI8.multiply(this.numerator, otherParsed.denominator),
  ...
  multiply(other) {
    const otherParsed = _Fraction.tryParseFraction(other);
    return new _Fraction(
      JSBI8.multiply(this.numerator, otherParsed.numerator),
      JSBI8.multiply(this.denominator, otherParsed.denominator)
    );
  }
  divide(other) {
    ...
    return new _Fraction(
      JSBI8.multiply(this.numerator, otherParsed.denominator),
      JSBI8.multiply(this.denominator, otherParsed.numerator)
    );
  }
  toSignificant(significantDigits, format = { groupSeparator: "" }, rounding = 1 /* ROUND_HALF_UP */) {
    invariant10(Number.isInteger(significantDigits), `${significantDigits} is not an integer.`);
    invariant10(significantDigits > 0, `${significantDigits} is not positive.`);
    Decimal.set({ precision: significantDigits + 1, rounding: toSignificantRounding[rounding] });
    const quotient = new Decimal(this.numerator.toString()).div(this.denominator.toString()).toSignificantDigits(significantDigits);
    return quotient.toFormat(quotient.decimalPlaces(), format);
  }
  toFixed(decimalPlaces, format = { groupSeparator: "" }, rounding = 1 /* ROUND_HALF_UP */) {
    invariant10(Number.isInteger(decimalPlaces), `${decimalPlaces} is not an integer.`);
    invariant10(decimalPlaces >= 0, `${decimalPlaces} is negative.`);
...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant10(JSBI8.lessThanOrEqual(this.quotient, MaxUint256), "AMOUNT");
    this.currency = currency;
    this.decimalScale = JSBI8.exponentiate(JSBI8.BigInt(10), JSBI8.BigInt(currency.decimals));
  }
  add(other) {
    invariant10(this.currency.equals(other.currency), "CURRENCY");
  ...
  toFixed(decimalPlaces = this.currency.decimals, format, rounding = 0 /* ROUND_DOWN */) {
    invariant10(decimalPlaces <= this.currency.decimals, "DECIMALS");
...
var ONE_HUNDRED = new Fraction(JSBI8.BigInt(100));
...
var Price = class _Price extends Fraction {
  ...
  constructor(...args) {
    ...
    this.scalar = new Fraction(
      JSBI8.exponentiate(JSBI8.BigInt(10), JSBI8.BigInt(baseCurrency.decimals)),
      JSBI8.exponentiate(JSBI8.BigInt(10), JSBI8.BigInt(quoteCurrency.decimals))
    );
  }
  /**
   ...
   */
  multiply(other) {
    invariant10(this.quoteCurrency.equals(other.baseCurrency), "TOKEN");
  ...
  quote(currencyAmount) {
    invariant10(currencyAmount.currency.equals(this.baseCurrency), "TOKEN");
...
var BaseCurrency = class {
  ...
  constructor(chainId, decimals, symbol, name) {
    invariant10(Number.isSafeInteger(chainId), "CHAIN_ID");
    invariant10(decimals >= 0 && decimals < 255 && Number.isInteger(decimals), "DECIMALS");
...
var Token = class extends BaseCurrency {
  ...
  constructor(chainId, address, decimals, symbol, name, bypassChecksum, buyFeeBps, sellFeeBps) {
    ...
    if (buyFeeBps) {
      invariant10(buyFeeBps.gte(BigNumber.from(0)), "NON-NEGATIVE FOT FEES");
    }
    if (sellFeeBps) {
      invariant10(sellFeeBps.gte(BigNumber.from(0)), "NON-NEGATIVE FOT FEES");
    }
   ...
   */
  sortsBefore(other) {
    invariant10(this.chainId === other.chainId, "CHAIN_IDS");
    invariant10(this.address.toLowerCase() !== other.address.toLowerCase(), "ADDRESSES");
...
function sortedInsert(items, add, maxSize, comparator) {
  invariant10(maxSize > 0, "MAX_SIZE_ZERO");
  invariant10(items.length <= maxSize, "ITEMS_SIZE");
  if (items.length === 0) {
    items.push(add);
    return null;
  } else {
...
var MAX_SAFE_INTEGER = JSBI8.BigInt(Number.MAX_SAFE_INTEGER);
var ZERO = JSBI8.BigInt(0);
var ONE = JSBI8.BigInt(1);
var TWO = JSBI8.BigInt(2);
function sqrt(value) {
  invariant10(JSBI8.greaterThanOrEqual(value, ZERO), "NEGATIVE");
  if (JSBI8.lessThan(value, MAX_SAFE_INTEGER)) {
    return JSBI8.BigInt(Math.floor(Math.sqrt(JSBI8.toNumber(value))));
  }
  let z;
  let x;
  z = value;
  x = JSBI8.add(JSBI8.divide(value, TWO), ONE);
  while (JSBI8.lessThan(x, z)) {
    z = x;
    x = JSBI8.divide(JSBI8.add(JSBI8.divide(value, x), x), TWO);
  }
  return z;
}
...
var MINIMUM_LIQUIDITY = JSBI8.BigInt(1e3);
var ZERO2 = JSBI8.BigInt(0);
var ONE2 = JSBI8.BigInt(1);
var FIVE = JSBI8.BigInt(5);
var _997 = JSBI8.BigInt(997);
var _1000 = JSBI8.BigInt(1e3);
var BASIS_POINTS = JSBI8.BigInt(1e4);
...
var Pair = class _Pair {
   ...
   */
  priceOf(token) {
    invariant10(this.involvesToken(token), "TOKEN");
  ...
  getOutputAmount(inputAmount, calculateFotFees = true) {
    invariant10(this.involvesToken(inputAmount.currency), "TOKEN");
    if (JSBI8.equal(this.reserve0.quotient, ZERO2) || JSBI8.equal(this.reserve1.quotient, ZERO2)) {
    ...
    const inputAmountWithFeeAndAfterTax = JSBI8.multiply(inputAmountAfterTax.quotient, _997);
    const numerator = JSBI8.multiply(inputAmountWithFeeAndAfterTax, outputReserve.quotient);
    const denominator = JSBI8.add(JSBI8.multiply(inputReserve.quotient, _1000), inputAmountWithFeeAndAfterTax);
    const outputAmount = CurrencyAmount.fromRawAmount(
      inputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
      JSBI8.divide(numerator, denominator)
      // JSBI.divide will round down by itself, which is desired
    );
    if (JSBI8.equal(outputAmount.quotient, ZERO2)) {
    ...
    if (JSBI8.equal(outputAmountAfterTax.quotient, ZERO2)) {
      throw new InsufficientInputAmountError();
    }
    return [
  ...
  getInputAmount(outputAmount, calculateFotFees = true) {
    invariant10(this.involvesToken(outputAmount.currency), "TOKEN");
    const percentAfterBuyFees = calculateFotFees ? this.derivePercentAfterBuyFees(outputAmount) : ZERO_PERCENT;
    const outputAmountBeforeTax = percentAfterBuyFees.greaterThan(ZERO_PERCENT) ? CurrencyAmount.fromRawAmount(
      outputAmount.currency,
      JSBI8.add(outputAmount.divide(percentAfterBuyFees).quotient, ONE2)
      // add 1 for rounding up
    ) : outputAmount;
    if (JSBI8.equal(this.reserve0.quotient, ZERO2) || JSBI8.equal(this.reserve1.quotient, ZERO2) || JSBI8.greaterThanOrEqual(outputAmount.quotient, this.reserveOf(outputAmount.currency).quotient) || JSBI8.greaterThanOrEqual(outputAmountBeforeTax.quotient, this.reserveOf(outputAmount.currency).quotient)) {
      throw new InsufficientReservesError();
    }
    const outputReserve = this.reserveOf(outputAmount.currency);
    const inputReserve = this.reserveOf(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0);
    const numerator = JSBI8.multiply(JSBI8.multiply(inputReserve.quotient, outputAmountBeforeTax.quotient), _1000);
    const denominator = JSBI8.multiply(JSBI8.subtract(outputReserve.quotient, outputAmountBeforeTax.quotient), _997);
    const inputAmount = CurrencyAmount.fromRawAmount(
      outputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
      JSBI8.add(JSBI8.divide(numerator, denominator), ONE2)
    ...
    const inputAmountBeforeTax = percentAfterSellFees.greaterThan(ZERO_PERCENT) ? CurrencyAmount.fromRawAmount(
      inputAmount.currency,
      JSBI8.add(inputAmount.divide(percentAfterSellFees).quotient, ONE2)
      // add 1 for rounding up
    ) : inputAmount;
    return [inputAmountBeforeTax, new _Pair(inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))];
  }
  getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    invariant10(totalSupply.currency.equals(this.liquidityToken), "LIQUIDITY");
    ...
    invariant10(tokenAmounts[0].currency.equals(this.token0) && tokenAmounts[1].currency.equals(this.token1), "TOKEN");
    let liquidity;
    if (JSBI8.equal(totalSupply.quotient, ZERO2)) {
      liquidity = JSBI8.subtract(
        sqrt(JSBI8.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)),
        MINIMUM_LIQUIDITY
      );
    } else {
      const amount0 = JSBI8.divide(JSBI8.multiply(tokenAmounts[0].quotient, totalSupply.quotient), this.reserve0.quotient);
      const amount1 = JSBI8.divide(JSBI8.multiply(tokenAmounts[1].quotient, totalSupply.quotient), this.reserve1.quotient);
      liquidity = JSBI8.lessThanOrEqual(amount0, amount1) ? amount0 : amount1;
    }
    if (!JSBI8.greaterThan(liquidity, ZERO2)) {
      throw new InsufficientInputAmountError();
    }
  ...
  getLiquidityValue(token, totalSupply, liquidity, feeOn = false, kLast) {
    invariant10(this.involvesToken(token), "TOKEN");
    invariant10(totalSupply.currency.equals(this.liquidityToken), "TOTAL_SUPPLY");
    invariant10(liquidity.currency.equals(this.liquidityToken), "LIQUIDITY");
    invariant10(JSBI8.lessThanOrEqual(liquidity.quotient, totalSupply.quotient), "LIQUIDITY");
    let totalSupplyAdjusted;
    if (!feeOn) {
      totalSupplyAdjusted = totalSupply;
    } else {
      invariant10(!!kLast, "K_LAST");
      const kLastParsed = JSBI8.BigInt(kLast);
      if (!JSBI8.equal(kLastParsed, ZERO2)) {
        const rootK = sqrt(JSBI8.multiply(this.reserve0.quotient, this.reserve1.quotient));
        const rootKLast = sqrt(kLastParsed);
        if (JSBI8.greaterThan(rootK, rootKLast)) {
          const numerator = JSBI8.multiply(totalSupply.quotient, JSBI8.subtract(rootK, rootKLast));
    ...
    return CurrencyAmount.fromRawAmount(
      token,
      JSBI8.divide(JSBI8.multiply(liquidity.quotient, this.reserveOf(token).quotient), totalSupplyAdjusted.quotient)
    );
  }
  derivePercentAfterSellFees(inputAmount) {
    ...
    if (sellFeeBips?.gt(BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new Percent(JSBI8.BigInt(sellFeeBips)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  }
  derivePercentAfterBuyFees(outputAmount) {
    ...
    if (buyFeeBps?.gt(BigNumber.from(0))) {
      return ONE_HUNDRED_PERCENT.subtract(new Percent(JSBI8.BigInt(buyFeeBps)).divide(BASIS_POINTS));
    } else {
      return ZERO_PERCENT;
    }
  }
...
var Route = class {
  constructor(pairs, input, output) {
    this._midPrice = null;
    invariant10(pairs.length > 0, "PAIRS");
    ...
    invariant10(pairs[0].involvesToken(wrappedInput), "INPUT");
    invariant10(typeof output === "undefined" || pairs[pairs.length - 1].involvesToken(output.wrapped), "OUTPUT");
    const path = [wrappedInput];
    for (const [i, pair] of pairs.entries()) {
      const currentInput = path[i];
      invariant10(currentInput.equals(pair.token0) || currentInput.equals(pair.token1), "PATH");
...
function inputOutputComparator(a, b) {
  invariant10(a.inputAmount.currency.equals(b.inputAmount.currency), "INPUT_CURRENCY");
  invariant10(a.outputAmount.currency.equals(b.outputAmount.currency), "OUTPUT_CURRENCY");
...
var Trade = class _Trade {
  ...
  constructor(route, amount, tradeType) {
    ...
    if (tradeType === 0 /* EXACT_INPUT */) {
      invariant10(amount.currency.equals(route.input), "INPUT");
      ...
      invariant10(amount.currency.equals(route.output), "OUTPUT");
  ...
  minimumAmountOut(slippageTolerance) {
    invariant10(!slippageTolerance.lessThan(ZERO2), "SLIPPAGE_TOLERANCE");
  ...
  static bestTradeExactIn(pairs, currencyAmountIn, currencyOut, { maxNumResults = 3, maxHops = 3 } = {}, currentPairs = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
    invariant10(pairs.length > 0, "PAIRS");
    invariant10(maxHops > 0, "MAX_HOPS");
    invariant10(currencyAmountIn === nextAmountIn || currentPairs.length > 0, "INVALID_RECURSION");
  ...
  static bestTradeExactOut(pairs, currencyIn, currencyAmountOut, { maxNumResults = 3, maxHops = 3 } = {}, currentPairs = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
    invariant10(pairs.length > 0, "PAIRS");
    invariant10(maxHops > 0, "MAX_HOPS");
    invariant10(currencyAmountOut === nextAmountOut || currentPairs.length > 0, "INVALID_RECURSION");
