...
import { toUtf8Bytes as toUtf8Bytes$1 } from '@ethersproject/strings';
import { getAddress } from '@ethersproject/address';
import { hexZeroPad, isBytes, hexlify, isHexString, hexConcat, hexStripZeros, arrayify } from '@ethersproject/bytes';
import invariant8 from 'tiny-invariant';
import { BigNumber as BigNumber$1, ethers, constants, ContractFactory, utils, Contract } from 'ethers';
...
var TypedDataEncoder = class _TypedDataEncoder {
  ...
  static hash(domain, types, value) {
    return keccak256$1(_TypedDataEncoder.encode(domain, types, value));
  }
...
var SignatureTransfer = class _SignatureTransfer {
  ...
  static getPermitData(permit, permit2Address, chainId, witness) {
    invariant8(MaxSigDeadline.gte(permit.deadline), "SIG_DEADLINE_OUT_OF_RANGE");
    invariant8(MaxUnorderedNonce.gte(permit.nonce), "NONCE_OUT_OF_RANGE");
...
function validateTokenPermissions(permissions) {
  invariant8(MaxSignatureTransferAmount.gte(permissions.amount), "AMOUNT_OUT_OF_RANGE");
}
var MaxUint2563 = JSBI2.BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
var RoundingMode = {
  RoundDown: 0,
  RoundHalfUp: 1,
  RoundUp: 3
...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant8(JSBI2.lessThanOrEqual(this.quotient, MaxUint2563), "AMOUNT");
...
var BPS = 1e4;
var MPS = BigNumber$1.from(10).pow(7);
...
var STRICT_EXCLUSIVITY = BigNumber$1.from(0);
...
var DutchOrder = class _DutchOrder {
  ...
  static fromJSON(json, chainId, _permit2Address) {
    return new _DutchOrder(
      {
        ...json,
        exclusivityOverrideBps: BigNumber$1.from(json.exclusivityOverrideBps),
        nonce: BigNumber$1.from(json.nonce),
        input: {
          token: json.input.token,
          startAmount: BigNumber$1.from(json.input.startAmount),
          endAmount: BigNumber$1.from(json.input.endAmount)
        },
        outputs: json.outputs.map((output) => ({
          token: output.token,
          startAmount: BigNumber$1.from(output.startAmount),
          endAmount: BigNumber$1.from(output.endAmount),
   ...
   */
  resolve(options) {
    const useOverride = this.info.exclusiveFiller !== ethers.constants.AddressZero && options.timestamp <= this.info.decayStartTime && options.filler !== this.info.exclusiveFiller;
    return {
      input: {
        token: this.info.input.token,
...
var ExclusiveDutchOrderReactor__factory = class extends ContractFactory {
  constructor(...args) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
...
var OrderQuoter__factory = class extends ContractFactory {
  constructor(...args) {
...
var OrderQuoterV4__factory = class extends ContractFactory {
  constructor(...args) {
...
var Permit2__factory = class extends ContractFactory {
  constructor(...args) {
...
var Proxy__factory = class extends ContractFactory {
  constructor(...args) {
...
var RelayOrderReactor__factory = class extends ContractFactory {
  constructor(...args) {
...
var NonceManager = class {
  ...
  async isUsed(address, nonce) {
    ...
    return bitmap.div(BigNumber$1.from(2).pow(bitPos)).mod(2).eq(1);
  }
  // Returns the first word that contains empty bits
  async getNextOpenWord(address) {
    let currentWord = this.currentWord.get(address) || BigNumber$1.from(0);
...
function getFirstUnsetBit(bitmap) {
  for (let i = 0; i < 256; i++) {
    if (bitmap.div(BigNumber$1.from(2).pow(i)).mod(2).eq(0)) {
      return i;
    }
  }
  return -1;
...
function setBit(bitmap, bitPos) {
  const mask = BigNumber$1.from(2).pow(bitPos);
...
function getCancelMultipleParams(noncesToCancel) {
  ...
  return Object.entries(splitNoncesByWord).map(([word, splitNonce2]) => {
    let mask = BigNumber$1.from(0);
    splitNonce2.forEach((splitNonce3) => {
      mask = mask.or(BigNumber$1.from(2).pow(splitNonce3.bitPos));
    });
    return { word: BigNumber$1.from(word), mask };
  });
}
...
async function multicall(provider, calls, stateOverrides, blockOverrides) {
  ...
  if (code.length > 2) {
    ...
    const deploylessInterface = new Interface(deploylessMulticall2_default);
  ...
  const multicallInterface = new Interface(multicall2_default);
          ...
          if (returnData.includes(key)) {
            if (key === "0a0b0d79") {
              if ("additionalValidationData" in orders[idx].order.info) {
                ...
                if (fillerValidation.type === 1 /* ExclusiveFiller */ && fillerValidation.data.filler !== ethers.constants.AddressZero) {
...
var EventWatcher = class {
  ...
  getTokenTransfers(logs, recipient) {
    const ERC20Interface = new utils.Interface(MockERC20_default.abi);
...
var NonLinearDutchDecayLib = class {
  ...
  static linearDecay(startPoint, endPoint, currentPoint, startAmount, endAmount) {
    ...
    const elapsed = BigNumber$1.from(currentPoint - startPoint);
    const duration = BigNumber$1.from(endPoint - startPoint);
    let delta;
    if (endAmount.lt(startAmount)) {
      delta = BigNumber$1.from(0).sub(startAmount.sub(endAmount).mul(elapsed).div(duration));
    } else {
...
var UnsignedV3DutchOrder = class _UnsignedV3DutchOrder {
    ...
    return new _UnsignedV3DutchOrder(
      {
        ...json,
        nonce: BigNumber$1.from(json.nonce),
        startingBaseFee: BigNumber$1.from(json.startingBaseFee),
        input: {
          ...json.input,
          startAmount: BigNumber$1.from(json.input.startAmount),
          curve: {
          ...
          maxAmount: BigNumber$1.from(json.input.maxAmount),
          adjustmentPerGweiBaseFee: BigNumber$1.from(
        ...
        outputs: json.outputs.map((output) => ({
          ...
          minAmount: BigNumber$1.from(output.minAmount),
          adjustmentPerGweiBaseFee: BigNumber$1.from(
  ...
  hash() {
    ...
    return ethers.utils._TypedDataEncoder.from(V3_DUTCH_ORDER_TYPES).hash(witnessInfo);
  }
  /**
    ...
    return new _CosignedV3DutchOrder(
      {
        ...
        cosignerData: {
          ...
          exclusivityOverrideBps: BigNumber$1.from(
            json.cosignerData.exclusivityOverrideBps
          ),
          inputOverride: BigNumber$1.from(json.cosignerData.inputOverride),
...
function encodeRelativeBlocks(relativeBlocks) {
  let packedData = BigNumber$1.from(0);
  for (let i = 0; i < relativeBlocks.length; i++) {
    packedData = packedData.or(BigNumber$1.from(relativeBlocks[i]).shl(i * 16));
  }
  return packedData;
}
...
var UnsignedPriorityOrder = class _UnsignedPriorityOrder {
    ...
    return new _UnsignedPriorityOrder(
      {
        ...
        auctionStartBlock: BigNumber$1.from(json.auctionStartBlock),
        baselinePriorityFeeWei: BigNumber$1.from(json.baselinePriorityFeeWei),
        nonce: BigNumber$1.from(json.nonce),
        input: {
          token: json.input.token,
          amount: BigNumber$1.from(json.input.amount),
          mpsPerPriorityFeeWei: BigNumber$1.from(json.input.mpsPerPriorityFeeWei)
        },
        outputs: json.outputs.map((output) => ({
          token: output.token,
          amount: BigNumber$1.from(output.amount),
          mpsPerPriorityFeeWei: BigNumber$1.from(output.mpsPerPriorityFeeWei),
          recipient: output.recipient
        }))
      },
      chainId,
   ...
   */
  hash() {
    return ethers.utils._TypedDataEncoder.from(PRIORITY_ORDER_TYPES).hash(this.witnessInfo());
  }
  /**
   * Returns the resolved order with the given options
   * @return The resolved order
    ...
    return new _CosignedPriorityOrder(
      {
        ...
        cosignerData: {
          auctionTargetBlock: BigNumber$1.from(
    ...
    return new _RelayOrder(
      {
        ...
        fee: {
          ...
          startAmount: BigNumber$1.from(json.fee.startAmount),
          endAmount: BigNumber$1.from(json.fee.endAmount),
...
var UnsignedV2DutchOrder = class _UnsignedV2DutchOrder {
   ...
   */
  hash() {
    return ethers.utils._TypedDataEncoder.from(V2_DUTCH_ORDER_TYPES).hash(this.witnessInfo());
  }
  /**
   * Returns the resolved order with the given options
   * @return The resolved order
...
var ORDER_INFO_V4_TYPE_HASH = ethers.utils.keccak256(
...
var HYBRID_INPUT_TYPE_HASH = ethers.utils.keccak256(
...
var HYBRID_OUTPUT_TYPE_HASH = ethers.utils.keccak256(
...
var HYBRID_ORDER_TYPE_HASH = ethers.utils.keccak256(
...
function hashPriceCurve(curve) {
  ...
  const values = curve.map((value) => BigNumber$1.from(value));
...
var FEED_INFO_TYPE_HASH = ethers.utils.keccak256(
...
var PRIVATE_INTENT_TYPE_HASH = ethers.utils.keccak256(
...
var OUTPUT_ALLOCATION_TYPE_HASH = ethers.utils.keccak256(
...
var DCA_INTENT_TYPE_HASH = ethers.utils.keccak256(
...
var DCA_COSIGNER_DATA_TYPE_HASH = ethers.utils.keccak256(
...
var BASE_SCALING_FACTOR = ethers.constants.WeiPerEther;

// src/uniswapx/order/v4/HybridOrder.ts
var ZERO_ADDRESS = ethers.constants.AddressZero;
var WAD = ethers.constants.WeiPerEther;
var MAX_UINT_240 = BigNumber$1.from(1).shl(240).sub(1);
...
var UnsignedHybridOrder = class _UnsignedHybridOrder {
    ...
    return new _UnsignedHybridOrder(
      {
        ...
        auctionStartBlock: BigNumber$1.from(json.auctionStartBlock),
        baselinePriorityFee: BigNumber$1.from(json.baselinePriorityFee),
        scalingFactor: BigNumber$1.from(json.scalingFactor),
        priceCurve: json.priceCurve.map((value) => BigNumber$1.from(value))
      },
      chainId,
      resolver,
      _permit2Address
  ...
  hash() {
    return hashHybridOrder({
      ...this.info,
      cosignerData: {
        auctionTargetBlock: BigNumber$1.from(0)}});
  }
  serialize() {
    const abiCoder = new ethers.utils.AbiCoder();
...
var CosignedHybridOrder = class _CosignedHybridOrder extends UnsignedHybridOrder {
  ...
  resolve(options) {
    ...
    if (this.info.cosigner !== ZERO_ADDRESS) {
      ...
      if (ethers.utils.getAddress(recovered) !== ethers.utils.getAddress(this.info.cosigner)) {
    ...
    const priorityFeeAboveBaseline = options.priorityFeeWei.gt(
      this.info.baselinePriorityFee
    ) ? options.priorityFeeWei.sub(this.info.baselinePriorityFee) : BigNumber$1.from(0);
...
function applySupplementalPriceCurve(priceCurve, supplemental) {
  if (supplemental.length === 0) {
    return priceCurve.map((value) => BigNumber$1.from(value));
  }
  ...
  const combined = priceCurve.map((value) => BigNumber$1.from(value));
  ...
  for (let i = 0; i < length; i++) {
    ...
    const supplementalScaling = BigNumber$1.from(supplemental[i]);
...
function locateCurrentAmount(startAmount, endAmount, startBlock, currentBlock, endBlock, roundUp) {
  ...
  const durationBN = BigNumber$1.from(duration);
  const elapsedBN = BigNumber$1.from(elapsed);
  const remainingBN = BigNumber$1.from(remaining);
...
function encodePriceCurveElement(duration, scalingFactor) {
  return BigNumber$1.from(duration).shl(PRICE_CURVE_DURATION_SHIFT).or(scalingFactor);
}
...
var OrderBuilder = class {
  ...
  getOrderInfo() {
    invariant8(this.orderInfo.reactor !== void 0, "reactor not set");
    invariant8(this.orderInfo.nonce !== void 0, "nonce not set");
    invariant8(this.orderInfo.deadline !== void 0, "deadline not set");
    invariant8(
      this.orderInfo.deadline > Date.now() / 1e3,
      `Deadline must be in the future: ${this.orderInfo.deadline}`
    );
    invariant8(this.orderInfo.swapper !== void 0, "swapper not set");
    invariant8(
...
var DutchOrderBuilder = class _DutchOrderBuilder extends OrderBuilder {
  constructor(chainId, reactorAddress, permit2Address) {
    ...
    this.info = {
      ...
      exclusivityOverrideBps: BigNumber$1.from(0)
    };
  }
  ...
  build() {
    invariant8(this.info.decayStartTime !== void 0, "decayStartTime not set");
    invariant8(this.info.input !== void 0, "input not set");
    invariant8(this.info.decayEndTime !== void 0, "decayEndTime not set");
    invariant8(
...
var RelayOrderBuilder = class _RelayOrderBuilder {
  ...
  feeStartTime(feeStartTime) {
    invariant8(this.info.fee !== void 0, "fee not set");
    this.info.fee = {
      ...this.info.fee,
  ...
  build() {
    invariant8(this.info.reactor !== void 0, "reactor not set");
    invariant8(this.info.nonce !== void 0, "nonce not set");
    invariant8(this.info.deadline !== void 0, "deadline not set");
    invariant8(
      this.info.deadline > Date.now() / 1e3,
      `Deadline must be in the future: ${this.info.deadline}`
    );
    invariant8(this.info.swapper !== void 0, "swapper not set");
    invariant8(
...
var V2DutchOrderBuilder = class _V2DutchOrderBuilder extends OrderBuilder {
  constructor(chainId, reactorAddress, _permit2Address) {
    ...
    this.info = {
      ...
      cosignerData: {
        ...
        exclusiveFiller: ethers.constants.AddressZero,
        exclusivityOverrideBps: BigNumber$1.from(0),
        inputOverride: BigNumber$1.from(0),
        outputOverrides: []
      }
    };
  }
  ...
  buildPartial() {
    invariant8(this.info.cosigner !== void 0, "cosigner not set");
    invariant8(this.info.input !== void 0, "input not set");
    invariant8(
      this.info.outputs && this.info.outputs.length > 0,
      "outputs not set"
    );
    invariant8(this.info.input !== void 0, "original input not set");
    invariant8(
  ...
  build() {
    invariant8(this.info.cosigner !== void 0, "cosigner not set");
    invariant8(this.info.cosignature !== void 0, "cosignature not set");
    invariant8(this.info.input !== void 0, "input not set");
    invariant8(
      this.info.outputs && this.info.outputs.length > 0,
      "outputs not set"
    );
    invariant8(this.info.cosignerData !== void 0, "cosignerData not set");
    invariant8(
...
var V3DutchOrderBuilder = class _V3DutchOrderBuilder extends OrderBuilder {
  ...
  checkUnsignedInvariants(info) {
    invariant8(info.cosigner !== void 0, "cosigner not set");
    invariant8(info.startingBaseFee !== void 0, "startingBaseFee not set");
    invariant8(info.input !== void 0, "input not set");
    invariant8(info.outputs && info.outputs.length > 0, "outputs not set");
    invariant8(
  ...
  checkCosignedInvariants(info) {
    invariant8(info.cosignerData !== void 0, "cosignerData not set");
    invariant8(
...
var ZERO_ADDRESS2 = constants.AddressZero;
var HybridOrderBuilder = class _HybridOrderBuilder {
  ...
  auctionStartBlock(block) {
    this.orderData.auctionStartBlock = typeof block === "number" ? BigNumber$1.from(block) : block;
    return this;
  }
  baselinePriorityFee(fee) {
    this.orderData.baselinePriorityFee = typeof fee === "number" ? BigNumber$1.from(fee) : fee;
    return this;
  }
  ...
  auctionTargetBlock(block) {
    if (!this.orderData.cosignerData) {
      this.initializeCosignerData({
        auctionTargetBlock: typeof block === "number" ? BigNumber$1.from(block) : block
      });
    } else {
      this.orderData.cosignerData.auctionTargetBlock = typeof block === "number" ? BigNumber$1.from(block) : block;
    }
    return this;
  }
  ...
  checkUnsignedInvariants() {
    ...
    invariant8(this.orderData.input !== void 0, "input not set");
    invariant8(
    ...
    invariant8(this.orderData.priceCurve !== void 0, "priceCurve not set");
  ...
  extractScalingFactor(curveElement) {
    const mask = BigNumber$1.from(2).pow(240).sub(1);
...
export { BPS, CosignedHybridOrder, CosignedPriorityOrder, CosignedV2DutchOrder, CosignedV3DutchOrder, DCA_COSIGNER_DATA_TYPE_HASH, DCA_INTENT_TYPES, DutchOrder, DutchOrderBuilder, DutchOrderTrade, EXCLUSIVE_FILLER_VALIDATION_MAPPING, HYBRID_ORDER_TYPES, HYBRID_RESOLVER_ADDRESS_MAPPING, HybridOrderBuilder, HybridOrderCosignatureError, HybridOrderPriceCurveError, HybridOrderTrade, KNOWN_EVENT_SIGNATURES, MPS, NonceManager, ORDER_INFO_V4_TYPE_HASH, ORDER_INFO_V4_TYPE_STRING, OrderBuilder, OrderNotFillable, OrderResolutionError, OrderType, OrderValidation, OrderValidator, PERMISSIONED_TOKENS, PERMIT2_MAPPING, PermissionedTokenInterface, PermissionedTokenProxyType, PermissionedTokenValidator, PriorityOrderBuilder, PriorityOrderTrade, REACTOR_ADDRESS_MAPPING, REACTOR_CONTRACT_MAPPING, RELAY_SENTINEL_RECIPIENT, REVERSE_REACTOR_MAPPING, REVERSE_RESOLVER_MAPPING, RelayEventWatcher, RelayOrder, RelayOrderBuilder, RelayOrderParser, RelayOrderQuoter, RelayOrderTrade, RelayOrderValidator, UNISWAPX_ORDER_QUOTER_MAPPING, UNISWAPX_V4_ORDER_QUOTER_MAPPING, UNISWAPX_V4_TOKEN_TRANSFER_HOOK_MAPPING, UniswapXEventWatcher, UniswapXOrderParser, UniswapXOrderQuoter, UnsignedHybridOrder, UnsignedPriorityOrder, UnsignedV2DutchOrder, UnsignedV3DutchOrder, V2DutchOrderBuilder, V2DutchOrderTrade, V3DutchOrderBuilder, V3DutchOrderTrade, V3_DUTCH_ORDER_TYPES, V4OrderQuoter, V4OrderValidator, ValidationType, buildNonce, constructSameAddressMap, encodeExclusiveFillerData, encodeRelativeBlocks, getCancelMultipleParams, getCancelSingleParams, getDecayedAmount, getFirstUnsetBit, getPermit2, getReactor, hashDCACosignerData, hashDCAIntent, hashHybridCosignerData, hashHybridOrder, hashOrderInfoV4, hashPrivateIntent, id2 as id, multicall, multicallAddressOn, multicallSameContractManyFunctions, multicallSameFunctionManyContracts, originalIfZero, parseExclusiveFillerData, parseValidation, setBit, splitNonce, stripHexPrefix };
