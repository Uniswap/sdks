...
describe('Claim Builders', () => {
    ...
    describe('ClaimBuilder factory', () => {
        it('should create a SingleClaimBuilder', () => {
            const builder = claim_1.ClaimBuilder.single(domain);
            expect(builder).toBeInstanceOf(claim_1.SingleClaimBuilder);
        });
        it('should create a BatchClaimBuilder', () => {
            const builder = claim_1.ClaimBuilder.batch(domain);
            expect(builder).toBeInstanceOf(claim_1.BatchClaimBuilder);
        });
    });
    describe('SingleClaimBuilder', () => {
        it('should build a claim with manual field setting', () => {
            const expires = BigInt(Date.now() + 3600000);
            const id = (0, locks_1.encodeLockId)(lockTag, usdcAddress);
            const claim = claim_1.ClaimBuilder.single(domain)
        ...
        it('should pre-fill from a compact', () => {
            ...
            expect(claim.struct.id).toBe((0, locks_1.encodeLockId)(lockTag, usdcAddress));
        });
        ...
        it('should allow different token in fromCompact', () => {
            ...
            const claim = claim_1.ClaimBuilder.single(domain)
                ...
                .addTransfer({ recipient: recipientAddress, amount: 1000000n })
                .build();
            expect(claim.struct.id).toBe((0, locks_1.encodeLockId)(lockTag, daiAddress));
        });
        ...
        it('should throw if sponsor is missing', () => {
            expect(() => claim_1.ClaimBuilder.single(domain)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .id((0, locks_1.encodeLockId)(lockTag, usdcAddress))
                .allocatedAmount(1000000n)
        ...
        it('should throw if allocatedAmount is missing', () => {
            expect(() => claim_1.ClaimBuilder.single(domain)
                ...
                .lockTag(lockTag)
                .addTransfer({ recipient: recipientAddress, amount: 1000000n })
                .build()).toThrow('allocatedAmount is required');
        });
        it('should throw if no claimants are added', () => {
            expect(() => claim_1.ClaimBuilder.single(domain)
                ...
                .allocatedAmount(1000000n)
                .lockTag(lockTag)
                .build()).toThrow('at least one claimant is required');
        });
        it('should throw if adding claimant without lockTag', () => {
            expect(() => claim_1.ClaimBuilder.single(domain)
                ...
                .id((0, locks_1.encodeLockId)(lockTag, usdcAddress))
                .allocatedAmount(1000000n)
                .addTransfer({ recipient: recipientAddress, amount: 1000000n })).toThrow('lockTag must be set before adding claimants');
        });
    });
    describe('BatchClaimBuilder', () => {
        it('should build a batch claim with manual field setting', () => {
            const expires = BigInt(Date.now() + 3600000);
            const id1 = (0, locks_1.encodeLockId)(lockTag, usdcAddress);
            const id2 = (0, locks_1.encodeLockId)(lockTag, '0x6b175474e89094c44da98b954eedeac495271d0f');
            const claim = claim_1.ClaimBuilder.batch(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(expires)
                .addClaim()
        ...
        it('should pre-fill from a batch compact', () => {
            ...
            const claim = claim_1.ClaimBuilder.batch(domain)
                ...
                .done()
                .addClaim()
                .id((0, locks_1.encodeLockId)(lockTag2, usdcAddress))
                .allocatedAmount(2000000n)
                .addPortion(lockTag2, {
                kind: 'transfer',
                recipient: recipientAddress,
            ...
            expect(claim.struct.expires).toBe(compact.expires);
            expect(claim.struct.claims.length).toBe(2);
            expect(claim.struct.claims[0].id).toBe((0, locks_1.encodeLockId)(lockTag, usdcAddress));
        });
        ...
        it('should throw if sponsor is missing', () => {
            expect(() => claim_1.ClaimBuilder.batch(domain)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .addClaim()
                .id(100n)
        ...
        it('should throw if no claimants are added', () => {
            expect(() => claim_1.ClaimBuilder.batch(domain)
                ...
                .id(100n)
                .allocatedAmount(1000000n)
                .done()).toThrow('at least one portion is required');
        });
    });
    describe('MultichainClaimBuilder', () => {
        it('should build a multichain claim with single resource', () => {
            ...
            const claim = claim_1.ClaimBuilder.multichain(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(expires)
                .id(resourceId)
        ...
        it('should throw if sponsor is missing', () => {
            expect(() => claim_1.ClaimBuilder.multichain(domain)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .id(100n)
                .lockTag(lockTag)
    ...
    describe('BatchMultichainClaimBuilder', () => {
        it('should build a batch multichain claim with multiple resources', () => {
            ...
            const claim = claim_1.ClaimBuilder.batchMultichain(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(expires)
                .addClaim()
        ...
        it('should throw if no claims are added', () => {
            expect(() => claim_1.ClaimBuilder.batchMultichain(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .build()).toThrow('at least one claim component is required');
        ...
        it('should throw if claim component is missing allocatedAmount', () => {
            expect(() => claim_1.ClaimBuilder.batchMultichain(domain)
                ...
                .done()).toThrow('allocatedAmount is required');
        });
    ...
    describe('complex claim example', () => {
        it('should build a multi-recipient claim with convert and withdraw', () => {
            ...
            const recipient2 = '0x9876543210987654321098765432109876543210';
            const targetLockTag = '0x000000000000000000000002';
            const claim = claim_1.ClaimBuilder.single(domain)
                .fromCompact({ compact, signature })
                .allocatedAmount(1000000n)
        ...
        it('should build a cross-chain multichain claim with hash references', () => {
            const expires = BigInt(Date.now() + 3600000);
            const resourceId = (0, locks_1.encodeLockId)(lockTag, usdcAddress);
            const polygonChainHash = '0x1234567890123456789012345678901234567890123456789012345678901234';
            const claim = claim_1.ClaimBuilder.multichain(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(expires)
    ...
    describe('EIP-712 hash and typed data', () => {
        describe('SingleClaimBuilder', () => {
            ...
            it('should generate different hashes for different claims', () => {
                const claim1 = claim_1.ClaimBuilder.single(domain)
                    .sponsor(sponsorAddress)
                    .nonce(1n)
                    .expires(BigInt(Date.now() + 3600000))
                    .id((0, locks_1.encodeLockId)(lockTag, usdcAddress))
                    ...
                    .addTransfer({ recipient: recipientAddress, amount: 1000000n })
                    .build();
                const claim2 = claim_1.ClaimBuilder.single(domain)
                    .sponsor(sponsorAddress)
                    .nonce(2n)
                    .expires(BigInt(Date.now() + 3600000))
                    .id((0, locks_1.encodeLockId)(lockTag, usdcAddress))
