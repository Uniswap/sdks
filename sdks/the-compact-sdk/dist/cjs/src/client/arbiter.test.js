...
describe('ArbiterClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new arbiter_1.ArbiterClient(config);
        jest.clearAllMocks();
    });
    describe('claim()', () => {
        it('should submit a valid claim', async () => {
            ...
            expect(result.claimHash).toBe((0, hashes_1.claimHash)(claim));
        });
        it('should throw if walletClient is missing', async () => {
            const clientWithoutWallet = new arbiter_1.ArbiterClient({
                ...config,
        ...
        it('should throw if contract address is missing', async () => {
            const clientWithoutAddress = new arbiter_1.ArbiterClient({
                ...config,
                address: undefined,
            });
            const claim = {
    ...
    describe('batchClaim()', () => {
        it('should submit a valid batch claim', async () => {
            ...
            expect(result.txHash).toBe(txHash);
            expect(result.claimHash).toMatch(/^0x[0-9a-f]{64}$/);
            expect(result.claimHash).toBe((0, hashes_1.batchClaimHash)(claim));
        });
        it('should throw if walletClient is missing', async () => {
            const clientWithoutWallet = new arbiter_1.ArbiterClient({
                ...config,
