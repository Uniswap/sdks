...
describe('ViewClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new view_1.ViewClient(config);
        jest.clearAllMocks();
    });
    describe('getLockDetails()', () => {
        it('should get lock details', async () => {
            const lockId = (0, locks_1.encodeLockId)('0x123456789012345678901234', tokenAddress);
            // Mock contract response [token, allocator, resetPeriod, scope]
            mockPublicClient.readContract.mockResolvedValue([
                tokenAddress,
                allocatorAddress,
                runtime_1.ResetPeriod.TenMinutes,
                runtime_1.Scope.Multichain,
            ]);
            ...
            expect(result.resetPeriod).toBe(runtime_1.ResetPeriod.TenMinutes);
            expect(result.scope).toBe(runtime_1.Scope.Multichain);
            expect(result.lockTag).toBe('0x123456789012345678901234');
        });
        it('should throw if contract address is missing', async () => {
            const clientWithoutAddress = new view_1.ViewClient({
                ...config,
                address: undefined,
            });
