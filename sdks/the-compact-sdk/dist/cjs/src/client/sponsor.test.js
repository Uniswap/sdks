...
const mandate_1 = require("../builders/mandate");
...
function createTransferEvent(params) {
    const transferEvent = theCompact_1.theCompactAbi.find((item) => item.type === 'event' && item.name === 'Transfer');
    (0, tiny_invariant_1.default)(transferEvent, 'Transfer event not found');
    const topics = (0, viem_1.encodeEventTopics)({
        abi: [transferEvent],
        eventName: 'Transfer',
        args: {
            from: params.from,
...
function createForcedWithdrawalStatusUpdatedEvent(params) {
    const event = theCompact_1.theCompactAbi.find((item) => item.type === 'event' && item.name === 'ForcedWithdrawalStatusUpdated');
    (0, tiny_invariant_1.default)(event, 'ForcedWithdrawalStatusUpdated event not found');
    const topics = (0, viem_1.encodeEventTopics)({
        abi: [event],
...
describe('SponsorClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new sponsor_1.SponsorClient(config);
        jest.clearAllMocks();
    });
    describe('depositNative()', () => {
        ...
        it('should throw if walletClient is missing', async () => {
            const clientWithoutWallet = new sponsor_1.SponsorClient({
                ...config,
    ...
    describe('builder factories', () => {
        ...
        describe('multichainCompact()', () => {
            it('should create a MultichainCompactBuilder with correct domain', () => {
                const arbiterAddress = '0xabcdefabcdefabcdefabcdefabcdefabcdefabcd';
                const mandateType = (0, mandate_1.simpleMandate)([{ name: 'fillerAddress', type: 'address' }]);
