...
const LOCK_TYPEHASH = (0, viem_1.keccak256)('0x4c6f636b28627974657331322 lockTag2c6164647265737320746f6b656e2c75696e7432353620616d6f756e7429'); // "Lock(bytes12 lockTag,address token,uint256 amount)"
describe('hash computation', () => {
    describe('claimHash', () => {
        it('should compute hash for a basic claim', () => {
            ...
            const hash = (0, hashes_1.claimHash)(claim);
            // Verify it produces a valid hash
            expect(hash).toMatch(/^0x[0-9a-f]{64}$/);
            // Verify it matches manual computation
            const expected = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([
        ...
        it('should produce different hashes for different nonces', () => {
            ...
            const hash1 = (0, hashes_1.claimHash)(baseClaim);
            const hash2 = (0, hashes_1.claimHash)(claim2);
        ...
        it('should handle claims with different witness values', () => {
            ...
            const claim2 = {
                ...claim1,
                witness: '0x2222222222222222222222222222222222222222222222222222222222222222',
            };
            const hash1 = (0, hashes_1.claimHash)(claim1);
            const hash2 = (0, hashes_1.claimHash)(claim2);
            expect(hash1).not.toBe(hash2);
        });
    });
    describe('batchClaimHash', () => {
        it('should compute hash for a batch claim with single id', () => {
            ...
            const hash = (0, hashes_1.batchClaimHash)(claim);
            // Verify it produces a valid hash
            expect(hash).toMatch(/^0x[0-9a-f]{64}$/);
            // Verify it matches manual computation
            const idsAndAmounts = claim.claims.map((c) => ({ id: c.id, amount: c.allocatedAmount }));
            const idsAndAmountsHashValue = (0, hashes_1.idsAndAmountsHash)(idsAndAmounts);
            const expected = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([
                { name: 'sponsor', type: 'address' },
                { name: 'nonce', type: 'uint256' },
                { name: 'expires', type: 'uint256' },
        ...
        it('should produce different hashes for different idsAndAmounts', () => {
            ...
            const hash1 = (0, hashes_1.batchClaimHash)(baseClaim);
            const hash2 = (0, hashes_1.batchClaimHash)(claim2);
            expect(hash1).not.toBe(hash2);
        });
    ...
    describe('componentsHash', () => {
        it('should hash empty components array', () => {
            const components = [];
            const hash = (0, hashes_1.componentsHash)(components);
            // Empty array should hash to keccak256('0x')
            const expected = (0, viem_1.keccak256)('0x');
            expect(hash).toBe(expected);
        });
        it('should hash single component', () => {
            ...
            const componentHash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([
                { name: 'claimant', type: 'uint256' },
                { name: 'amount', type: 'uint256' }
            ], [claimant, amount]));
            const expected = (0, viem_1.keccak256)(componentHash);
            expect(hash).toBe(expected);
        });
        it('should hash multiple components', () => {
            ...
            const hash1 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[0].claimant, components[0].amount]));
            const hash2 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[1].claimant, components[1].amount]));
            const hash3 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[2].claimant, components[2].amount]));
            const expected = (0, viem_1.keccak256)((0, viem_1.encodePacked)(['bytes32', 'bytes32', 'bytes32'], [hash1, hash2, hash3]));
            expect(hash).toBe(expected);
        });
    });
    describe('idsAndAmountsHash', () => {
        it('should hash empty ids and amounts array', () => {
            const idsAndAmounts = [];
            const hash = (0, hashes_1.idsAndAmountsHash)(idsAndAmounts);
            // Empty array should hash to keccak256('0x')
            const expected = (0, viem_1.keccak256)('0x');
            expect(hash).toBe(expected);
        });
        it('should hash single id and amount pair', () => {
            ...
            const pairHash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([
                { name: 'id', type: 'uint256' },
                { name: 'amount', type: 'uint256' }
            ], [id, amount]));
            const expected = (0, viem_1.keccak256)(pairHash);
            expect(hash).toBe(expected);
        });
        it('should hash multiple id and amount pairs', () => {
            ...
            const hash = (0, hashes_1.idsAndAmountsHash)(idsAndAmounts);
            // Expected: keccak256(concat(hash1, hash2, hash3))
            const hash1 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[0].id, idsAndAmounts[0].amount]));
            const hash2 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[1].id, idsAndAmounts[1].amount]));
            const hash3 = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[2].id, idsAndAmounts[2].amount]));
            const expected = (0, viem_1.keccak256)((0, viem_1.encodePacked)(['bytes32', 'bytes32', 'bytes32'], [hash1, hash2, hash3]));
            expect(hash).toBe(expected);
        });
        it('should handle well-known token IDs (from ClaimHashLib.t.sol)', () => {
            ...
            const pairHash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [id, amount]));
            const expected = (0, viem_1.keccak256)(pairHash);
            expect(hash).toBe(expected);
            expect(hash).toMatch(/^0x[0-9a-f]{64}$/);
        });
        it('should handle multiple well-known tokens (from ClaimHashLib.t.sol)', () => {
            ...
            const hashes = idsAndAmounts.map(pair => (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [pair.id, pair.amount])));
            const expected = (0, viem_1.keccak256)((0, viem_1.encodePacked)(['bytes32', 'bytes32', 'bytes32'], hashes));
            expect(hash).toBe(expected);
        });
    });
    describe('edge cases', () => {
        ...
        it('should handle zero values', () => {
            ...
            const hash = (0, hashes_1.componentsHash)(components);
            const componentHash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [0n, 0n]));
            const expected = (0, viem_1.keccak256)(componentHash);
            expect(hash).toBe(expected);
        });
        it('should handle large arrays', () => {
            ...
            const smallerHash = (0, hashes_1.componentsHash)(components.slice(0, 50));
    ...
    describe('hash consistency', () => {
        it('should produce consistent hashes for identical inputs', () => {
            ...
            const hash1 = (0, hashes_1.componentsHash)(components);
            const hash2 = (0, hashes_1.componentsHash)(components);
            const hash3 = (0, hashes_1.componentsHash)([...components]); // New array, same values
        ...
        it('should produce different hashes for different inputs', () => {
            ...
            const hash1 = (0, hashes_1.componentsHash)(components1);
            const hash2 = (0, hashes_1.componentsHash)(components2);
            const hash3 = (0, hashes_1.componentsHash)(components3);
            expect(hash1).not.toBe(hash2);
    ...
    describe('compatibility with Solidity implementation', () => {
        it('should match HashLib.sol empty array hash', () => {
            ...
            const hash = (0, hashes_1.idsAndAmountsHash)([]);
        ...
        it('should match lock hash structure from The Compact', () => {
            ...
            const lockTypehash = (0, viem_1.keccak256)('0x4c6f636b28627974657331322 lockTag2c6164647265737320746f6b656e2c75696e7432353620616d6f756e7429');
            const lockHash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)([
...
//# sourceMappingURL=hashes.test.js.map