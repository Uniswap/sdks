...
exports.SCALING_FACTOR = exports.InvalidPriceCurveParametersError = exports.PriceCurveBlocksExceededError = void 0;
exports.createPriceCurveElement = createPriceCurveElement;
exports.getBlockDuration = getBlockDuration;
exports.getScalingFactor = getScalingFactor;
exports.unpackPriceCurveElement = unpackPriceCurveElement;
exports.sharesScalingDirection = sharesScalingDirection;
exports.applySupplementalPriceCurve = applySupplementalPriceCurve;
exports.getCalculatedScalingFactor = getCalculatedScalingFactor;
exports.createPriceCurve = createPriceCurve;
exports.unpackPriceCurve = unpackPriceCurve;
/**
 ...
 * Error types
 */
class PriceCurveBlocksExceededError extends Error {
    constructor() {
        super('Price curve blocks exceeded');
        this.name = 'PriceCurveBlocksExceededError';
    }
}
exports.PriceCurveBlocksExceededError = PriceCurveBlocksExceededError;
class InvalidPriceCurveParametersError extends Error {
    constructor(message) {
        super(message || 'Invalid price curve parameters');
        this.name = 'InvalidPriceCurveParametersError';
    }
}
exports.InvalidPriceCurveParametersError = InvalidPriceCurveParametersError;
/**
 ...
 */
function createPriceCurveElement(blockDuration, scalingFactor) {
    if (blockDuration < 0 || blockDuration > MAX_BLOCK_DURATION) {
        throw new InvalidPriceCurveParametersError(`Block duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${blockDuration}`);
    }
    if (scalingFactor < 0n || scalingFactor > MAX_SCALING_FACTOR) {
        throw new InvalidPriceCurveParametersError(`Scaling factor must be between 0 and ${MAX_SCALING_FACTOR}, got ${scalingFactor}`);
    }
 ...
 */
function getScalingFactor(element) {
 ...
 */
function unpackPriceCurveElement(element) {
    return {
 ...
 */
function applySupplementalPriceCurve(baseCurve, supplementalCurve) {
    const combinedCurve = [];
    ...
    for (let i = 0; i < applicationRange; i++) {
        ...
        if (!sharesScalingDirection(scalingFactor, supplementalScalingFactor)) {
            throw new InvalidPriceCurveParametersError(`Incompatible scaling directions at index ${i}: base=${scalingFactor}, supplemental=${supplementalScalingFactor}`);
        }
        // Check for overflow (must fit in 240 bits)
        if (combinedScalingFactor > MAX_SCALING_FACTOR || combinedScalingFactor < 0n) {
            throw new InvalidPriceCurveParametersError(`Combined scaling factor overflow at index ${i}: ${combinedScalingFactor}`);
        }
 ...
 */
function getCalculatedScalingFactor(curve, blocksPassed) {
    ...
    for (let i = 0; i < curve.length; i++) {
        ...
        if (blocksPassed < blocksCounted + blockDuration) {
            ...
            let startScalingFactor;
            let endScalingFactor;
            ...
            if (hasPassedZeroDuration && i > 0 && getBlockDuration(curve[i - 1]) === 0) {
                ...
                if (!sharesScalingDirection(startScalingFactor, endScalingFactor)) {
                    throw new InvalidPriceCurveParametersError('Incompatible scaling directions between zero-duration segment and following segment');
                }
                return locateCurrentAmount(startScalingFactor, endScalingFactor, blocksCounted, blocksPassed, blocksCounted + blockDuration, startScalingFactor > 1000000000000000000n // Round up for exact-in
                );
            }
            else {
 ...
 */
function createPriceCurve(elements) {
...
//# sourceMappingURL=priceCurve.js.map