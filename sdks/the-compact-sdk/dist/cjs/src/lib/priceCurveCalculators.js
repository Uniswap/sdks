...
exports.percentToScalingFactor = percentToScalingFactor;
exports.scalingFactorToPercent = scalingFactorToPercent;
exports.basisPointsToScalingFactor = basisPointsToScalingFactor;
exports.createDutchAuction = createDutchAuction;
exports.createReverseDutchAuction = createReverseDutchAuction;
exports.createSteppedPrice = createSteppedPrice;
exports.createSteppedPriceSchedule = createSteppedPriceSchedule;
exports.getPriceAtBlock = getPriceAtBlock;
exports.getCurveDuration = getCurveDuration;
exports.validatePriceCurve = validatePriceCurve;
exports.concatenateCurves = concatenateCurves;
exports.addPlateau = addPlateau;
exports.createFlatPrice = createFlatPrice;
exports.createMultiPhaseAuction = createMultiPhaseAuction;
 ...
 */
function percentToScalingFactor(percent) {
    if (!Number.isFinite(percent) || percent < 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Percent must be a finite non-negative number, got ${percent}`);
    }
 ...
 */
function scalingFactorToPercent(scalingFactor) {
    if (scalingFactor < 0n) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Scaling factor must be non-negative, got ${scalingFactor}`);
    }
 ...
 */
function basisPointsToScalingFactor(basisPoints) {
    if (!Number.isFinite(basisPoints) || basisPoints < 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Basis points must be a finite non-negative number, got ${basisPoints}`);
    }
 ...
 * })
 */
function createDutchAuction(config) {
    const { startPricePercent, endPricePercent, durationBlocks, steps = 1 } = config;
    // Validate inputs
    if (startPricePercent < 1.0 || endPricePercent < 1.0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Dutch auction requires percentages >= 1.0 (exact-in mode), got start=${startPricePercent}, end=${endPricePercent}`);
    }
    if (startPricePercent < endPricePercent) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Dutch auction must have decreasing prices: start (${startPricePercent}) should be >= end (${endPricePercent})`);
    }
    if (durationBlocks <= 0 || durationBlocks > MAX_BLOCK_DURATION * steps) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Duration must be between 1 and ${MAX_BLOCK_DURATION * steps} blocks, got ${durationBlocks}`);
    }
    if (steps < 1 || !Number.isInteger(steps)) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Steps must be a positive integer, got ${steps}`);
    }
    ...
    if (steps === 1) {
        ...
        if (endScaling === NEUTRAL_SCALING || startScaling === endScaling) {
            return [(0, priceCurve_1.createPriceCurveElement)(durationBlocks, startScaling)];
        }
        else {
            // Need two elements: duration segment + zero-duration end target for interpolation
            return [(0, priceCurve_1.createPriceCurveElement)(durationBlocks, startScaling), (0, priceCurve_1.createPriceCurveElement)(0, endScaling)];
        }
    }
    ...
    for (let i = 0; i < steps; i++) {
        ...
        if (i > 0) {
            curve.push((0, priceCurve_1.createPriceCurveElement)(0, stepScaling));
        }
        // Add duration for this step (distribute remainder to early steps)
        const duration = segmentDuration + (i < remainder ? 1 : 0);
        curve.push((0, priceCurve_1.createPriceCurveElement)(duration, stepScaling));
    }
    // Validate all segments share scaling direction
    for (let i = 0; i < curve.length - 1; i++) {
        const currentScaling = (0, priceCurve_1.unpackPriceCurveElement)(curve[i]).scalingFactor;
        const nextScaling = (0, priceCurve_1.unpackPriceCurveElement)(curve[i + 1]).scalingFactor;
        if (!(0, priceCurve_1.sharesScalingDirection)(currentScaling, nextScaling)) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Incompatible scaling directions at segment ${i}: ${currentScaling} vs ${nextScaling}`);
        }
    }
    return curve;
}
 ...
 * })
 */
function createReverseDutchAuction(config) {
    const { startPricePercent, endPricePercent, durationBlocks, steps = 1 } = config;
    // Validate inputs
    if (startPricePercent > 1.0 || endPricePercent > 1.0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Reverse Dutch auction requires percentages <= 1.0 (exact-out mode), got start=${startPricePercent}, end=${endPricePercent}`);
    }
    if (startPricePercent > endPricePercent) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Reverse Dutch auction must have increasing prices: start (${startPricePercent}) should be <= end (${endPricePercent})`);
    }
    if (durationBlocks <= 0 || durationBlocks > MAX_BLOCK_DURATION * steps) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Duration must be between 1 and ${MAX_BLOCK_DURATION * steps} blocks, got ${durationBlocks}`);
    }
 ...
 * })
 */
function createSteppedPrice(config) {
    const { steps } = config;
    if (!steps || steps.length === 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError('Stepped price requires at least one step');
    }
    const curve = [];
    for (let i = 0; i < steps.length; i++) {
        const step = steps[i];
        if (step.blockDuration < 0 || step.blockDuration > MAX_BLOCK_DURATION) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Block duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${step.blockDuration} at step ${i}`);
        }
        ...
        if (step.blockDuration === 0 && i > 0) {
            ...
            curve.push((0, priceCurve_1.createPriceCurveElement)(0, scaling));
        }
        else if (step.blockDuration > 0) {
            // Add the duration segment
            curve.push((0, priceCurve_1.createPriceCurveElement)(step.blockDuration, scaling));
        }
        else if (i === 0) {
    ...
    for (let i = 0; i < curve.length - 1; i++) {
        ...
        const nextScaling = (0, priceCurve_1.unpackPriceCurveElement)(curve[i + 1]).scalingFactor;
        if (!(0, priceCurve_1.sharesScalingDirection)(currentScaling, nextScaling)) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Incompatible scaling directions between steps: exact-in and exact-out modes cannot be mixed`);
        }
    }
    return curve;
}
 ...
 * })
 */
function createSteppedPriceSchedule(config) {
    const { startPricePercent, schedule } = config;
    if (!schedule || schedule.length === 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError('Schedule must contain at least one price change');
    }
    // Sort schedule by block number
    const sortedSchedule = [...schedule].sort((a, b) => a.atBlock - b.atBlock);
    // Validate schedule
    if (sortedSchedule[0].atBlock < 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError('Schedule blocks must be non-negative');
    }
    const curve = [];
    let currentBlock = 0;
    ...
    for (const change of sortedSchedule) {
        ...
        if (blockDuration < 0) {
            throw new priceCurve_1.InvalidPriceCurveParametersError('Schedule must have increasing block numbers');
        }
        if (blockDuration > MAX_BLOCK_DURATION) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Segment duration ${blockDuration} exceeds maximum ${MAX_BLOCK_DURATION}`);
        }
        // Add segment from current position to change point
        if (blockDuration > 0) {
            const scaling = percentToScalingFactor(currentPrice);
            curve.push((0, priceCurve_1.createPriceCurveElement)(blockDuration, scaling));
        }
        // Handle transition
        if (change.instant) {
            // Instant jump: add zero-duration element
            const newScaling = percentToScalingFactor(change.pricePercent);
            curve.push((0, priceCurve_1.createPriceCurveElement)(0, newScaling));
        }
    ...
    for (let i = 0; i < curve.length - 1; i++) {
        ...
        const nextScaling = (0, priceCurve_1.unpackPriceCurveElement)(curve[i + 1]).scalingFactor;
        if (!(0, priceCurve_1.sharesScalingDirection)(currentScaling, nextScaling)) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Incompatible scaling directions in schedule: exact-in and exact-out modes cannot be mixed`);
        }
    }
    return curve;
}
 ...
 */
function getPriceAtBlock(curve, blocksPassed) {
    if (blocksPassed < 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError('Blocks passed must be non-negative');
    }
    try {
        const scalingFactor = (0, priceCurve_1.getCalculatedScalingFactor)(curve, blocksPassed);
...
function getCurveDuration(curve) {
    ...
    for (const element of curve) {
        const { blockDuration } = (0, priceCurve_1.unpackPriceCurveElement)(element);
        totalDuration += blockDuration;
        if (totalDuration > Number.MAX_SAFE_INTEGER) {
            throw new priceCurve_1.InvalidPriceCurveParametersError('Total curve duration exceeds maximum safe integer');
        }
    }
 ...
 * }
 */
function validatePriceCurve(curve) {
    const errors = [];
    const warnings = [];
    ...
    let previousScaling = null;
    for (let i = 0; i < curve.length; i++) {
        try {
            const { blockDuration, scalingFactor } = (0, priceCurve_1.unpackPriceCurveElement)(curve[i]);
            ...
            if (previousScaling !== null) {
                if (!(0, priceCurve_1.sharesScalingDirection)(previousScaling, scalingFactor)) {
 ...
 */
function concatenateCurves(segments) {
    ...
    if (!validation.valid) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Concatenated curve is invalid: ${validation.errors?.join(', ')}`);
    }
    return combined;
}
/**
 ...
 */
function addPlateau(curve, plateauBlocks) {
    if (plateauBlocks < 0 || plateauBlocks > MAX_BLOCK_DURATION) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Plateau duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${plateauBlocks}`);
    }
    if (plateauBlocks === 0) {
        return [...curve];
    }
    if (curve.length === 0) {
        ...
        return [(0, priceCurve_1.createPriceCurveElement)(plateauBlocks, NEUTRAL_SCALING)];
    }
    // Get final scaling factor from curve
    const lastElement = curve[curve.length - 1];
    const { scalingFactor } = (0, priceCurve_1.unpackPriceCurveElement)(lastElement);
    // Add plateau at the final price
    return [...curve, (0, priceCurve_1.createPriceCurveElement)(plateauBlocks, scalingFactor)];
}
/**
 ...
 */
function createFlatPrice(pricePercent, durationBlocks) {
    if (durationBlocks < 0 || durationBlocks > MAX_BLOCK_DURATION) {
        throw new priceCurve_1.InvalidPriceCurveParametersError(`Duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${durationBlocks}`);
    }
    const scaling = percentToScalingFactor(pricePercent);
    return [(0, priceCurve_1.createPriceCurveElement)(durationBlocks, scaling)];
}
/**
 ...
 * })
 */
function createMultiPhaseAuction(config) {
    const { startPricePercent, phases } = config;
    if (!phases || phases.length === 0) {
        throw new priceCurve_1.InvalidPriceCurveParametersError('Multi-phase auction requires at least one phase');
    }
    const curve = [];
    let currentPrice = startPricePercent;
    for (let i = 0; i < phases.length; i++) {
        ...
        if (phase.durationBlocks < 0) {
            throw new priceCurve_1.InvalidPriceCurveParametersError(`Phase ${i}: duration must be non-negative, got ${phase.durationBlocks}`);
        }
        ...
        if (currentPrice === phase.endPricePercent) {
            // Flat price phase
            const scaling = percentToScalingFactor(currentPrice);
            curve.push((0, priceCurve_1.createPriceCurveElement)(phase.durationBlocks, scaling));
        }
            ...
            if (currentPrice >= 1.0 && phase.endPricePercent >= 1.0) {
                ...
                curve.push((0, priceCurve_1.createPriceCurveElement)(phase.durationBlocks, startScaling));
            }
        }
        else {
...
//# sourceMappingURL=priceCurveCalculators.js.map