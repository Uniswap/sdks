 ...
 */
import { getCalculatedScalingFactor, unpackPriceCurveElement, InvalidPriceCurveParametersError, SCALING_FACTOR, } from './priceCurve';
import { percentToScalingFactor, scalingFactorToPercent, basisPointsToScalingFactor, createDutchAuction, createReverseDutchAuction, createSteppedPrice, createSteppedPriceSchedule, getPriceAtBlock, getCurveDuration, validatePriceCurve, concatenateCurves, addPlateau, createFlatPrice, createMultiPhaseAuction, } from './priceCurveCalculators';
describe('Price Curve Calculators', () => {
    describe('percentage utilities', () => {
        describe('percentToScalingFactor', () => {
            it('should convert 100% to 1e18', () => {
                expect(percentToScalingFactor(1.0)).toBe(1000000000000000000n);
            });
            it('should convert 150% to 1.5e18', () => {
                expect(percentToScalingFactor(1.5)).toBe(1500000000000000000n);
            });
            it('should convert 50% to 0.5e18', () => {
                expect(percentToScalingFactor(0.5)).toBe(500000000000000000n);
            });
            it('should convert 90% to 0.9e18', () => {
                expect(percentToScalingFactor(0.9)).toBe(900000000000000000n);
            });
            it('should handle zero percent', () => {
                expect(percentToScalingFactor(0)).toBe(0n);
            });
            it('should throw on negative percent', () => {
                expect(() => percentToScalingFactor(-1)).toThrow(InvalidPriceCurveParametersError);
            });
            it('should throw on non-finite percent', () => {
                expect(() => percentToScalingFactor(NaN)).toThrow(InvalidPriceCurveParametersError);
                expect(() => percentToScalingFactor(Infinity)).toThrow(InvalidPriceCurveParametersError);
            });
            it('should handle decimal percentages with rounding', () => {
                const result = percentToScalingFactor(1.23456789);
        ...
        describe('scalingFactorToPercent', () => {
            it('should convert 1e18 to 100%', () => {
                expect(scalingFactorToPercent(1000000000000000000n)).toBe(1.0);
            });
            it('should convert 1.5e18 to 150%', () => {
                expect(scalingFactorToPercent(1500000000000000000n)).toBe(1.5);
            });
            it('should convert 0.5e18 to 50%', () => {
                expect(scalingFactorToPercent(500000000000000000n)).toBe(0.5);
            });
            it('should handle zero', () => {
                expect(scalingFactorToPercent(0n)).toBe(0);
            });
            it('should throw on negative scaling factor', () => {
                expect(() => scalingFactorToPercent(-1n)).toThrow(InvalidPriceCurveParametersError);
            });
            it('should be inverse of percentToScalingFactor', () => {
                const originalPercent = 1.25;
                const scaling = percentToScalingFactor(originalPercent);
                const roundTrip = scalingFactorToPercent(scaling);
                expect(roundTrip).toBeCloseTo(originalPercent, 10);
            });
        });
        describe('basisPointsToScalingFactor', () => {
            it('should convert 10000 bp to 1e18 (100%)', () => {
                expect(basisPointsToScalingFactor(10000)).toBe(1000000000000000000n);
            });
            it('should convert 15000 bp to 1.5e18 (150%)', () => {
                expect(basisPointsToScalingFactor(15000)).toBe(1500000000000000000n);
            });
            it('should convert 50 bp to 0.5% (0.005e18)', () => {
                expect(basisPointsToScalingFactor(50)).toBe(5000000000000000n);
            });
            it('should handle zero basis points', () => {
                expect(basisPointsToScalingFactor(0)).toBe(0n);
            });
            it('should throw on negative basis points', () => {
                expect(() => basisPointsToScalingFactor(-1)).toThrow(InvalidPriceCurveParametersError);
            });
            it('should throw on non-finite basis points', () => {
                expect(() => basisPointsToScalingFactor(NaN)).toThrow(InvalidPriceCurveParametersError);
            });
        });
    });
    describe('createDutchAuction', () => {
        it('should create linear Dutch auction (150% → 100%)', () => {
            ...
            const curve = createDutchAuction(config);
            expect(curve).toHaveLength(1);
            const element = unpackPriceCurveElement(curve[0]);
        ...
        it('should create linear Dutch auction with non-neutral end (200% → 150%)', () => {
            ...
            const midpointPrice = getPriceAtBlock(curve, 500);
        ...
        it('should create stepped Dutch auction with 5 steps', () => {
            ...
            // Verify first step
            const firstStep = unpackPriceCurveElement(curve[0]);
        ...
        it('should throw if start price < 1.0 (not exact-in)', () => {
            ...
            expect(() => createDutchAuction(config)).toThrow(InvalidPriceCurveParametersError);
        });
    ...
    describe('createReverseDutchAuction', () => {
        it('should create linear reverse Dutch auction (50% → 100%)', () => {
            ...
            const curve = createReverseDutchAuction(config);
            expect(curve).toHaveLength(1);
            const element = unpackPriceCurveElement(curve[0]);
            expect(element.blockDuration).toBe(1000);
        ...
        it('should throw if start price > 1.0 (not exact-out)', () => {
            ...
            expect(() => createReverseDutchAuction(config)).toThrow(InvalidPriceCurveParametersError);
        });
    ...
    describe('createSteppedPrice', () => {
        ...
        it('should handle instant transitions (zero duration)', () => {
            ...
            const instantElement = unpackPriceCurveElement(curve[1]);
        ...
        it('should throw on empty steps array', () => {
            expect(() => createSteppedPrice({ steps: [] })).toThrow(InvalidPriceCurveParametersError);
        });
        it('should throw if block duration exceeds maximum', () => {
            expect(() => createSteppedPrice({
                steps: [{ blockDuration: 70000, pricePercent: 1.0 }],
            })).toThrow(InvalidPriceCurveParametersError);
        });
    ...
    describe('createSteppedPriceSchedule', () => {
        it('should create schedule with instant transitions', () => {
            ...
            const duration = getCurveDuration(curve);
        ...
        it('should create schedule with linear transitions', () => {
            ...
            expect(curve.length).toBeGreaterThan(0);
            expect(getCurveDuration(curve)).toBe(100);
        });
        it('should throw on empty schedule', () => {
            expect(() => createSteppedPriceSchedule({
                startPricePercent: 1.5,
                schedule: [],
            })).toThrow(InvalidPriceCurveParametersError);
        });
    ...
    describe('getPriceAtBlock', () => {
        it('should return start price at block 0', () => {
            ...
            const price = getPriceAtBlock(curve, 0);
        ...
        it('should return interpolated price at midpoint', () => {
            ...
            const price = getPriceAtBlock(curve, 500);
        ...
        it('should return neutral price when blocks exceeded', () => {
            ...
            const price = getPriceAtBlock(curve, 2000);
        ...
        it('should throw on negative blocks', () => {
            ...
            expect(() => getPriceAtBlock(curve, -1)).toThrow(InvalidPriceCurveParametersError);
        });
        it('should handle empty curve', () => {
            const price = getPriceAtBlock([], 100);
    ...
    describe('getCurveDuration', () => {
        it('should return total duration for single segment', () => {
            ...
            expect(getCurveDuration(curve)).toBe(1000);
        });
        it('should sum durations for multiple segments', () => {
            ...
            expect(getCurveDuration(curve)).toBe(600);
        });
        it('should ignore zero-duration segments', () => {
            ...
            expect(getCurveDuration(curve)).toBe(300);
        });
        it('should return 0 for empty curve', () => {
            expect(getCurveDuration([])).toBe(0);
        });
    });
    describe('validatePriceCurve', () => {
        it('should validate valid Dutch auction curve', () => {
            ...
            const result = validatePriceCurve(curve);
        ...
        it('should warn on empty curve', () => {
            const result = validatePriceCurve([]);
            expect(result.valid).toBe(true);
    ...
    describe('concatenateCurves', () => {
        it('should concatenate multiple curve segments', () => {
            const phase1 = createDutchAuction({
                startPricePercent: 1.5,
                endPricePercent: 1.2,
                durationBlocks: 500,
            });
            const phase2 = createDutchAuction({
                startPricePercent: 1.2,
                endPricePercent: 1.0,
                durationBlocks: 500,
            });
            ...
            expect(getCurveDuration(combined)).toBe(1000);
        });
        it('should return empty array for empty input', () => {
            expect(concatenateCurves([])).toEqual([]);
        });
        it('should throw on invalid concatenation (mixed directions)', () => {
            const exactIn = createDutchAuction({
                startPricePercent: 1.5,
                endPricePercent: 1.0,
                durationBlocks: 500,
            });
            const exactOut = createReverseDutchAuction({
                startPricePercent: 0.5,
                endPricePercent: 1.0,
                durationBlocks: 500,
            });
            expect(() => concatenateCurves([exactIn, exactOut])).toThrow(InvalidPriceCurveParametersError);
        });
    });
    describe('addPlateau', () => {
        it('should add plateau to existing curve', () => {
            const baseCurve = createDutchAuction({
                startPricePercent: 1.5,
                endPricePercent: 1.2,
                durationBlocks: 500,
            });
            const withPlateau = addPlateau(baseCurve, 200);
            expect(withPlateau.length).toBe(baseCurve.length + 1);
            expect(getCurveDuration(withPlateau)).toBe(700);
        });
        it('should create plateau at neutral for empty curve', () => {
            const plateau = addPlateau([], 500);
            expect(plateau).toHaveLength(1);
            expect(getCurveDuration(plateau)).toBe(500);
            const element = unpackPriceCurveElement(plateau[0]);
            expect(element.scalingFactor).toBe(SCALING_FACTOR.NEUTRAL);
        });
        it('should return same curve for zero duration', () => {
            ...
            const result = addPlateau(baseCurve, 0);
        ...
        it('should throw on negative duration', () => {
            ...
            expect(() => addPlateau(baseCurve, -100)).toThrow(InvalidPriceCurveParametersError);
        });
    });
    describe('createFlatPrice', () => {
        it('should create flat price curve', () => {
            const curve = createFlatPrice(1.2, 1000);
            expect(curve).toHaveLength(1);
            const element = unpackPriceCurveElement(curve[0]);
            expect(element.blockDuration).toBe(1000);
            expect(element.scalingFactor).toBe(1200000000000000000n);
        ...
        it('should handle neutral price', () => {
            const curve = createFlatPrice(1.0, 500);
            const element = unpackPriceCurveElement(curve[0]);
            expect(element.scalingFactor).toBe(SCALING_FACTOR.NEUTRAL);
        });
        it('should throw on negative duration', () => {
            expect(() => createFlatPrice(1.0, -100)).toThrow(InvalidPriceCurveParametersError);
        });
    });
    describe('createMultiPhaseAuction', () => {
        it('should create multi-phase auction with 3 phases', () => {
            const curve = createMultiPhaseAuction({
                startPricePercent: 1.5,
                phases: [
        ...
        it('should throw on empty phases', () => {
            expect(() => createMultiPhaseAuction({
                startPricePercent: 1.5,
                phases: [],
            })).toThrow(InvalidPriceCurveParametersError);
        });
    ...
    describe('extensive validation - exact expected values', () => {
        describe('Dutch auction mathematical correctness', () => {
            ...
            it('should produce correct stepped behavior (150% → 100% with 5 steps over 1000 blocks)', () => {
                ...
                const price0 = getPriceAtBlock(curve, 0);
                const price100 = getPriceAtBlock(curve, 100);
                const price250 = getPriceAtBlock(curve, 250);
                const price500 = getPriceAtBlock(curve, 500);
                const price900 = getPriceAtBlock(curve, 900);
        ...
        describe('Reverse Dutch auction mathematical correctness', () => {
            ...
            it('should produce exact stepped values (50% → 100% with 4 steps over 800 blocks)', () => {
                ...
                const price0 = getCalculatedScalingFactor(curve, 0);
                const price100 = getCalculatedScalingFactor(curve, 100);
                const price200 = getCalculatedScalingFactor(curve, 200);
                const price400 = getCalculatedScalingFactor(curve, 400);
                const price600 = getCalculatedScalingFactor(curve, 600);
        ...
        describe('Stepped price exact values', () => {
            it('should produce exact manual step values', () => {
                ...
                const price0 = getCalculatedScalingFactor(curve, 0);
                const price50 = getCalculatedScalingFactor(curve, 50);
                const price100 = getCalculatedScalingFactor(curve, 100);
                const price200 = getCalculatedScalingFactor(curve, 200);
                const price300 = getCalculatedScalingFactor(curve, 300);
        ...
        describe('Multi-phase auction exact values', () => {
            it('should produce exact values for 3-phase auction', () => {
                ...
                const price0 = getCalculatedScalingFactor(curve, 0);
                const price200 = getCalculatedScalingFactor(curve, 200);
                const price399 = getCalculatedScalingFactor(curve, 399);
                expect(price0 > price200).toBe(true);
                expect(price200 > price399).toBe(true);
                expect(price0 > 1900000000000000000n).toBe(true); // ~2.0
                // Test phase 2: Flat at 1.5
                const price400 = getCalculatedScalingFactor(curve, 400);
                const price500 = getCalculatedScalingFactor(curve, 500);
                const price599 = getCalculatedScalingFactor(curve, 599);
                ...
                const price600 = getCalculatedScalingFactor(curve, 600);
                const price800 = getCalculatedScalingFactor(curve, 800);
                const price999 = getCalculatedScalingFactor(curve, 999);
    ...
    describe('integration with getCalculatedScalingFactor', () => {
        it('should work with Dutch auction curve', () => {
            ...
            // At start
            const startScaling = getCalculatedScalingFactor(curve, 0);
            expect(startScaling).toBe(1500000000000000000n);
            // At midpoint (approximate due to interpolation)
            const midScaling = getCalculatedScalingFactor(curve, 500);
        ...
        it('should work with Reverse Dutch curve', () => {
            ...
            const endScaling = getCalculatedScalingFactor(curve, 999);
        ...
        it('should work with stepped price curve', () => {
            ...
            const start = getCalculatedScalingFactor(curve, 0);
            expect(scalingFactorToPercent(start)).toBeCloseTo(1.5, 2);
            // After instant drop (well into second segment)
            const after = getCalculatedScalingFactor(curve, 150);
            expect(scalingFactorToPercent(after)).toBeCloseTo(1.2, 2);
        });
        it('should work with multi-phase auction', () => {
            ...
            // At start of phase 1
            const phase1Start = getCalculatedScalingFactor(curve, 0);
            expect(scalingFactorToPercent(phase1Start)).toBeCloseTo(1.5, 10);
            // At start of phase 2 (around block 500)
            const phase2Start = getCalculatedScalingFactor(curve, 500);
            expect(scalingFactorToPercent(phase2Start)).toBeCloseTo(1.2, 1);
        });
    });
});
