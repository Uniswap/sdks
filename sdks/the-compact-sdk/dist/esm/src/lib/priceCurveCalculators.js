 ...
 */
import { createPriceCurveElement, unpackPriceCurveElement, sharesScalingDirection, getCalculatedScalingFactor, InvalidPriceCurveParametersError, } from './priceCurve';
/**
 * Constants
 */
 ...
 */
export function percentToScalingFactor(percent) {
 ...
 */
export function scalingFactorToPercent(scalingFactor) {
    if (scalingFactor < 0n) {
 ...
 */
export function basisPointsToScalingFactor(basisPoints) {
 ...
 * })
 */
export function createDutchAuction(config) {
    const { startPricePercent, endPricePercent, durationBlocks, steps = 1 } = config;
    // Validate inputs
    if (startPricePercent < 1.0 || endPricePercent < 1.0) {
        throw new InvalidPriceCurveParametersError(`Dutch auction requires percentages >= 1.0 (exact-in mode), got start=${startPricePercent}, end=${endPricePercent}`);
    }
    if (startPricePercent < endPricePercent) {
        throw new InvalidPriceCurveParametersError(`Dutch auction must have decreasing prices: start (${startPricePercent}) should be >= end (${endPricePercent})`);
    }
    if (durationBlocks <= 0 || durationBlocks > MAX_BLOCK_DURATION * steps) {
        throw new InvalidPriceCurveParametersError(`Duration must be between 1 and ${MAX_BLOCK_DURATION * steps} blocks, got ${durationBlocks}`);
    }
    ...
    for (let i = 0; i < curve.length - 1; i++) {
        ...
        if (!sharesScalingDirection(currentScaling, nextScaling)) {
            throw new InvalidPriceCurveParametersError(`Incompatible scaling directions at segment ${i}: ${currentScaling} vs ${nextScaling}`);
        }
    }
    return curve;
}
 ...
 * })
 */
export function createReverseDutchAuction(config) {
    const { startPricePercent, endPricePercent, durationBlocks, steps = 1 } = config;
    // Validate inputs
    if (startPricePercent > 1.0 || endPricePercent > 1.0) {
        throw new InvalidPriceCurveParametersError(`Reverse Dutch auction requires percentages <= 1.0 (exact-out mode), got start=${startPricePercent}, end=${endPricePercent}`);
    }
    if (startPricePercent > endPricePercent) {
        throw new InvalidPriceCurveParametersError(`Reverse Dutch auction must have increasing prices: start (${startPricePercent}) should be <= end (${endPricePercent})`);
    }
    if (durationBlocks <= 0 || durationBlocks > MAX_BLOCK_DURATION * steps) {
        throw new InvalidPriceCurveParametersError(`Duration must be between 1 and ${MAX_BLOCK_DURATION * steps} blocks, got ${durationBlocks}`);
    }
 ...
 * })
 */
export function createSteppedPrice(config) {
    const { steps } = config;
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (step.blockDuration < 0 || step.blockDuration > MAX_BLOCK_DURATION) {
            throw new InvalidPriceCurveParametersError(`Block duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${step.blockDuration} at step ${i}`);
        }
    ...
    for (let i = 0; i < curve.length - 1; i++) {
        ...
        const nextScaling = unpackPriceCurveElement(curve[i + 1]).scalingFactor;
        if (!sharesScalingDirection(currentScaling, nextScaling)) {
            throw new InvalidPriceCurveParametersError(`Incompatible scaling directions between steps: exact-in and exact-out modes cannot be mixed`);
        }
    }
    return curve;
}
 ...
 * })
 */
export function createSteppedPriceSchedule(config) {
    ...
    for (const change of sortedSchedule) {
        ...
        if (blockDuration > MAX_BLOCK_DURATION) {
            throw new InvalidPriceCurveParametersError(`Segment duration ${blockDuration} exceeds maximum ${MAX_BLOCK_DURATION}`);
        }
    ...
    for (let i = 0; i < curve.length - 1; i++) {
        ...
        const nextScaling = unpackPriceCurveElement(curve[i + 1]).scalingFactor;
        if (!sharesScalingDirection(currentScaling, nextScaling)) {
            throw new InvalidPriceCurveParametersError(`Incompatible scaling directions in schedule: exact-in and exact-out modes cannot be mixed`);
        }
    }
    return curve;
}
 ...
 */
export function getPriceAtBlock(curve, blocksPassed) {
    if (blocksPassed < 0) {
 ...
 */
export function getCurveDuration(curve) {
    let totalDuration = 0;
 ...
 * }
 */
export function validatePriceCurve(curve) {
    const errors = [];
    const warnings = [];
 ...
 */
export function concatenateCurves(segments) {
 ...
 */
export function addPlateau(curve, plateauBlocks) {
    if (plateauBlocks < 0 || plateauBlocks > MAX_BLOCK_DURATION) {
        throw new InvalidPriceCurveParametersError(`Plateau duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${plateauBlocks}`);
    }
    if (plateauBlocks === 0) {
        return [...curve];
    }
 ...
 */
export function createFlatPrice(pricePercent, durationBlocks) {
    if (durationBlocks < 0 || durationBlocks > MAX_BLOCK_DURATION) {
        throw new InvalidPriceCurveParametersError(`Duration must be between 0 and ${MAX_BLOCK_DURATION}, got ${durationBlocks}`);
    }
 ...
 * })
 */
export function createMultiPhaseAuction(config) {
    ...
    for (let i = 0; i < phases.length; i++) {
        ...
        if (phase.durationBlocks < 0) {
            throw new InvalidPriceCurveParametersError(`Phase ${i}: duration must be non-negative, got ${phase.durationBlocks}`);
        }
