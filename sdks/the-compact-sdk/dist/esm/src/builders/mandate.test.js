...
describe('mandate', () => {
    describe('defineMandateType', () => {
        ...
        it('should encode and hash mandate values', () => {
            ...
            expect(hash).toEqual(keccak256(concat([orderId, encodeAbiParameters([{ name: 'minFill', type: 'uint256' }], [minFill])])));
        });
        it('should throw for nested types not starting with Mandate', () => {
            expect(() => defineMandateType({
    ...
    describe('hash computation', () => {
        ...
        it('should encode complex nested structures with arrays', () => {
            ...
            const itemTypehash = keccak256(itemTypestring);
            ...
            const expectedEncoded = concat([
                encodeAbiParameters([{ name: 'owner', type: 'address' }], [witnessValue.owner]),
                arrayHash,
            ]);
        ...
        it('should handle deeply nested structures with multiple levels', () => {
            ...
            const metaTypehash = keccak256(metaTypestring);
            ...
            const dataTypehash = keccak256(dataTypestring);
            const dataEncoded = concat([
                encodeAbiParameters([{ name: 'value', type: 'uint256' }], [witnessValue.data.value]),
                metaHash,
            ]);
            const dataHash = keccak256(concat([dataTypehash, dataEncoded]));
            // Step 3: Encode Mandate (includes dataHash)
            const expectedEncoded = concat([
                encodeAbiParameters([{ name: 'signer', type: 'address' }], [witnessValue.signer]),
                dataHash,
            ]);
            expect(encoded).toEqual(expectedEncoded);
            expect(hash).toEqual(keccak256(encoded));
        ...
        it('should match EIP-712 encoding for arrays of primitives', () => {
            ...
            const valuesEncoded = encodeAbiParameters([{ name: 'values', type: 'uint256[]' }], [witnessValue.values]);
            const valuesHash = keccak256(valuesEncoded);
            const expectedEncoded = concat([
                encodeAbiParameters([{ name: 'sender', type: 'address' }], [witnessValue.sender]),
                valuesHash,
            ]);
            expect(encoded).toEqual(expectedEncoded);
        });
    ...
    describe('typestring and typehash helpers', () => {
        ...
        it('should generate typehash matching keccak256 of typestring', () => {
            ...
            const expectedTypehash = keccak256('Mandate(uint256 witnessArgument)');
