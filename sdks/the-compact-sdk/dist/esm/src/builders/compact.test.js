...
const SimpleMandateType = simpleMandate([{ name: 'fillerAddress', type: 'address' }]);
describe('Compact Builders', () => {
    ...
    describe('CompactBuilder factory', () => {
        it('should create a SingleCompactBuilder', () => {
            const builder = CompactBuilder.single(domain);
            expect(builder).toBeInstanceOf(SingleCompactBuilder);
        });
        it('should create a BatchCompactBuilder', () => {
            const builder = CompactBuilder.batch(domain);
            expect(builder).toBeInstanceOf(BatchCompactBuilder);
        });
        it('should create a MultichainCompactBuilder', () => {
            const builder = CompactBuilder.multichain(domain);
            expect(builder).toBeInstanceOf(MultichainCompactBuilder);
        });
    });
    describe('SingleCompactBuilder', () => {
        ...
        it('should throw for invalid duration format', () => {
            expect(() => CompactBuilder.single(domain)
                .arbiter(arbiterAddress)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expiresIn('invalid')
    ...
    describe('BatchCompactBuilder', () => {
        ...
        it('should support expiresAt and expiresIn', () => {
            const expires = BigInt(Date.now() + 3600000);
            const result1 = CompactBuilder.batch(domain)
                .arbiter(arbiterAddress)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expiresAt(expires)
            ...
            const result2 = CompactBuilder.batch(domain)
                .arbiter(arbiterAddress)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expiresIn('10m')
        ...
        it('should throw if arbiter is missing', () => {
            expect(() => CompactBuilder.batch(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .addLock({ lockTag, token: tokenAddress, amount: 1000000n })
    ...
    describe('MultichainCompactBuilder', () => {
        ...
        it('should support expiresAt and expiresIn', () => {
            ...
            const result1 = CompactBuilder.multichain(domain)
                ...
                .build();
            expect(result1.struct.expires).toBe(expires);
            const result2 = CompactBuilder.multichain(domain)
                .sponsor(sponsorAddress)
                .nonce(1n)
                .expiresIn('30m')
                .addElement()
        ...
        it('should throw if sponsor is missing', () => {
            const mandateType = SimpleMandateType;
            const mandate = { fillerAddress: '0x9876543210987654321098765432109876543210' };
            expect(() => CompactBuilder.multichain(domain)
                .nonce(1n)
                .expires(BigInt(Date.now() + 3600000))
                .addElement()
                .arbiter(arbiterAddress)
