 ...
 */
export function defineMandateType(config) {
    ...
    if (nestedTypes) {
        for (const [typeName, typeFields] of Object.entries(nestedTypes)) {
            ...
            typeHashes.set(typeName, keccak256(typestring));
        }
    }
    return {
        name: 'Mandate',
        ...
        typehash() {
            return keccak256(this.typestring());
        },
        encode(value) {
...
function encodeStructValue(fields, value, nestedTypes, typeHashes) {
    ...
    for (const field of fields) {
        ...
        if (isArray && isPrimitiveType(baseType)) {
            ...
            const encoded = encodeAbiParameters([{ name: field.name, type: field.type }], [fieldValue]);
 ...
 * @returns A MandateType instance
 */
export function simpleMandate(fields) {
