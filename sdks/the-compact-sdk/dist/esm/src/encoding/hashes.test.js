...
const LOCK_TYPEHASH = keccak256('0x4c6f636b28627974657331322 lockTag2c6164647265737320746f6b656e2c75696e7432353620616d6f756e7429'); // "Lock(bytes12 lockTag,address token,uint256 amount)"
describe('hash computation', () => {
    ...
    describe('componentsHash', () => {
        ...
        it('should hash single component', () => {
            ...
            const componentHash = keccak256(encodeAbiParameters([
        ...
        it('should hash multiple components', () => {
            ...
            const hash1 = keccak256(encodeAbiParameters([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[0].claimant, components[0].amount]));
            const hash2 = keccak256(encodeAbiParameters([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[1].claimant, components[1].amount]));
            const hash3 = keccak256(encodeAbiParameters([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [components[2].claimant, components[2].amount]));
            const expected = keccak256(encodePacked(['bytes32', 'bytes32', 'bytes32'], [hash1, hash2, hash3]));
            expect(hash).toBe(expected);
        });
    });
    describe('idsAndAmountsHash', () => {
        ...
        it('should hash single id and amount pair', () => {
            ...
            const pairHash = keccak256(encodeAbiParameters([
                { name: 'id', type: 'uint256' },
                { name: 'amount', type: 'uint256' }
            ], [id, amount]));
        ...
        it('should hash multiple id and amount pairs', () => {
            ...
            const hash = idsAndAmountsHash(idsAndAmounts);
            // Expected: keccak256(concat(hash1, hash2, hash3))
            const hash1 = keccak256(encodeAbiParameters([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[0].id, idsAndAmounts[0].amount]));
            const hash2 = keccak256(encodeAbiParameters([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[1].id, idsAndAmounts[1].amount]));
            const hash3 = keccak256(encodeAbiParameters([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [idsAndAmounts[2].id, idsAndAmounts[2].amount]));
            const expected = keccak256(encodePacked(['bytes32', 'bytes32', 'bytes32'], [hash1, hash2, hash3]));
            expect(hash).toBe(expected);
        });
        it('should handle well-known token IDs (from ClaimHashLib.t.sol)', () => {
            ...
            const pairHash = keccak256(encodeAbiParameters([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [id, amount]));
            const expected = keccak256(pairHash);
            expect(hash).toBe(expected);
            expect(hash).toMatch(/^0x[0-9a-f]{64}$/);
        });
        it('should handle multiple well-known tokens (from ClaimHashLib.t.sol)', () => {
            ...
            const hashes = idsAndAmounts.map(pair => keccak256(encodeAbiParameters([{ name: 'id', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [pair.id, pair.amount])));
            const expected = keccak256(encodePacked(['bytes32', 'bytes32', 'bytes32'], hashes));
            expect(hash).toBe(expected);
        });
    });
    describe('edge cases', () => {
        ...
        it('should handle zero values', () => {
            ...
            const hash = componentsHash(components);
            const componentHash = keccak256(encodeAbiParameters([{ name: 'claimant', type: 'uint256' }, { name: 'amount', type: 'uint256' }], [0n, 0n]));
            const expected = keccak256(componentHash);
            expect(hash).toBe(expected);
        });
    ...
    describe('compatibility with Solidity implementation', () => {
        ...
        it('should match lock hash structure from The Compact', () => {
            ...
            const lockTypehash = keccak256('0x4c6f636b28627974657331322 lockTag2c6164647265737320746f6b656e2c75696e7432353620616d6f756e7429');
            const lockHash = keccak256(encodeAbiParameters([
