...
describe('createCompactClient', () => {
  describe('with explicit address', () => {
    it('should create a client with all sub-clients', () => {
      ...
      const client = createCompactClient(config)

      expect(client).toBeDefined()
      expect(client.sponsor).toBeInstanceOf(SponsorClient)
      expect(client.arbiter).toBeInstanceOf(ArbiterClient)
  ...
  describe('error cases', () => {
    it('should throw for unsupported chain without explicit address', () => {
      ...
      expect(() => createCompactClient(config)).toThrow(
        'No default deployment found for chain 999999. Please provide an address.'
      )
    })
  ...
  describe('sub-client functionality', () => {
    ...
    it('should create independent clients for different chains', () => {
      const mainnetConfig: CompactClientConfig = {
        chainId: 1,
        publicClient: mockPublicClient,
      }

      const baseConfig: CompactClientConfig = {
        chainId: 8453,
        publicClient: mockPublicClient,
      }

      const mainnetClient = createCompactClient(mainnetConfig)
      const baseClient = createCompactClient(baseConfig)
