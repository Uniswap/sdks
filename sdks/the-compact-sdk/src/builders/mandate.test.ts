...
describe('mandate', () => {
  describe('defineMandateType', () => {
    it('should define a custom mandate type', () => {
      const Mandate = defineMandateType<{ orderId: Hex; minFill: bigint; bossMode: boolean }>({
        fields: [MandateFields.bytes32('orderId'), MandateFields.uint256('minFill'), MandateFields.bool('bossMode')],
      })

      expect(Mandate.name).toBe('Mandate')
      expect(Mandate.fields).toContainEqual(MandateFields.bytes32('orderId'))
      expect(Mandate.fields).toContainEqual(MandateFields.uint256('minFill'))
      expect(Mandate.fields).toContainEqual(MandateFields.bool('bossMode'))
    ...
    it('should encode and hash mandate values', () => {
      const Mandate = simpleMandate([MandateFields.bytes32('orderId'), MandateFields.uint256('minFill')])

      const orderId = '0x0000000000000000000000000000000000000000000000000000000987654321' as Hex
      const minFill = 1000n
    ...
    it('should throw for nested types not starting with Mandate', () => {
      expect(() =>
        defineMandateType({
          fields: [MandateFields.bytes32('data')],
          nestedTypes: {
            Schmandate: [MandateFields.uint256('value')],
          },
        })
      ).toThrow()
    })
  ...
  describe('nested mandate types', () => {
    it('should handle simple nested structure', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.uint256('amount'), { name: 'data', type: 'Mandate_Info' }],
        nestedTypes: {
          Mandate_Info: [MandateFields.address('token'), MandateFields.uint256('value')],
        },
      })

      expect(Mandate.witnessTypestring).toBe(
        'uint256 amount,Mandate_Info data)Mandate_Info(address token,uint256 value'
      )
    })
    ...
    it('should handle arrays in nested structures', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'items', type: 'Mandate_Item[]' }],
        nestedTypes: {
          Mandate_Item: [MandateFields.uint256('id'), MandateFields.uint256('quantity')],
        },
      })

      expect(Mandate.witnessTypestring).toBe(
        'address owner,Mandate_Item[] items)Mandate_Item(uint256 id,uint256 quantity'
      )
    })
    ...
    it('should handle multiple nested types in alphabetical order', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'tokenData', type: 'Mandate_Token' }],
        nestedTypes: {
          Mandate_Token: [MandateFields.address('token'), { name: 'amount', type: 'Mandate_Amount' }],
          Mandate_Amount: [MandateFields.uint256('value'), MandateFields.uint256('decimals')],
        },
      })

      expect(Mandate.witnessTypestring).toBe(
        'address owner,Mandate_Token tokenData)Mandate_Amount(uint256 value,uint256 decimals)Mandate_Token(address token,Mandate_Amount amount'
      )
    })
    ...
    it('should handle deeply nested structure with recursive references', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'data', type: 'Mandate_Data' }],
        nestedTypes: {
          Mandate_Data: [MandateFields.uint256('value'), { name: 'compacts', type: 'Mandate_Compact[]' }],
          Mandate_Compact: [
      ...
      expect(Mandate.witnessTypestring).toBe(
        'address owner,Mandate_Data data)Mandate_Compact(address sponsor,uint256 nonce,Mandate mandate)Mandate_Data(uint256 value,Mandate_Compact[] compacts'
      )
    })
    ...
    it('should handle full Tribunal mandate types', () => {
      ...
      const expectedTypestring =
        'address adjuster,Mandate_Fill[] fills)' +
        'Mandate_BatchCompact(address arbiter,address sponsor,uint256 nonce,uint256 expires,Mandate_Lock[] commitments,Mandate mandate)' +
    ...
    it('should generate correct typestring for complex nested structures', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'data', type: 'Mandate_Data' }],
        nestedTypes: {
          Mandate_Data: [MandateFields.address('token'), MandateFields.uint256('amount')],
        },
      })

      // Should generate the correct witness typestring
      expect(Mandate.witnessTypestring).toBe(
        'address owner,Mandate_Data data)Mandate_Data(address token,uint256 amount'
      )
    ...
    it('should handle bytes types in nested structures', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('signer'), { name: 'callback', type: 'Mandate_Callback' }],
        nestedTypes: {
          Mandate_Callback: [MandateFields.uint256('chainId'), MandateFields.bytes('context')],
        },
      })

      expect(Mandate.witnessTypestring).toBe(
        'address signer,Mandate_Callback callback)Mandate_Callback(uint256 chainId,bytes context'
      )
    })
  ...
  describe('hash computation', () => {
    ...
    it('should encode complex nested structures with arrays', () => {
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'items', type: 'Mandate_Item[]' }],
        nestedTypes: {
          Mandate_Item: [MandateFields.uint256('id'), MandateFields.uint256('amount')],
        },
      })
      ...
      const item1Hash = keccak256(concat([itemTypehash, item1Encoded]))

      // Encode second item: abi.encode(2, 200)
      const item2Encoded = encodeAbiParameters(
        [
          { name: 'id', type: 'uint256' },
          { name: 'amount', type: 'uint256' },
        ] as any,
      ...
      const item2Hash = keccak256(concat([itemTypehash, item2Encoded]))

      // Array of structs: keccak256(hash1 || hash2)
      const arrayHash = keccak256(concat([item1Hash, item2Hash]))

      // Final encoding: owner || arrayHash
      const expectedEncoded = concat([
        encodeAbiParameters([{ name: 'owner', type: 'address' }] as any, [witnessValue.owner]),
        arrayHash,
      ])
    ...
    it('should handle deeply nested structures with multiple levels', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('signer'), { name: 'data', type: 'Mandate_Data' }],
        nestedTypes: {
          Mandate_Data: [MandateFields.uint256('value'), { name: 'meta', type: 'Mandate_Meta' }],
          Mandate_Meta: [MandateFields.address('token'), MandateFields.uint256('amount')],
        },
      })
      ...
      const metaHash = keccak256(concat([metaTypehash, metaEncoded]))
      ...
      const dataHash = keccak256(concat([dataTypehash, dataEncoded]))

      // Step 3: Encode Mandate (includes dataHash)
      const expectedEncoded = concat([
        encodeAbiParameters([{ name: 'signer', type: 'address' }] as any, [witnessValue.signer]),
        dataHash,
      ])
    ...
    it('should match EIP-712 encoding for arrays of primitives', () => {
      ...
      const Mandate = defineMandateType({
        fields: [MandateFields.address('sender'), { name: 'values', type: 'uint256[]' }],
      })
      ...
      const valuesEncoded = encodeAbiParameters([{ name: 'values', type: 'uint256[]' }] as any, [witnessValue.values])
      const valuesHash = keccak256(valuesEncoded)

      const expectedEncoded = concat([
        encodeAbiParameters([{ name: 'sender', type: 'address' }] as any, [witnessValue.sender]),
        valuesHash,
      ])
  ...
  describe('typestring and typehash helpers', () => {
    ...
    it('should generate full typestring for complex nested mandates', () => {
      const Mandate = defineMandateType({
        fields: [MandateFields.address('owner'), { name: 'data', type: 'Mandate_Data' }],
        nestedTypes: {
          Mandate_Data: [MandateFields.uint256('value'), MandateFields.address('token')],
        },
      })
  ...
  describe('Tribunal mandate encoding validation', () => {
    it('should encode mandate matching Tribunal test case', () => {
      ...
      const adjuster = '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' as Address
      const witnessValue = {
        adjuster,
        fills: [
          {
            chainId: 1n,
            tribunal: '0x5FbDB2315678afecb367f032d93F642f64180aa3' as Address,
            expires: 1703116800n,
            components: [
              {
                fillToken: '0x000000000000000000000000000000000000dead' as Address,
