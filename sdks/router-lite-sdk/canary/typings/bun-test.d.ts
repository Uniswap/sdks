 ...
 * `../integration/typings/bun-test.d.ts`). It exists so editors can type-check the canary suite
 * WITHOUT depending on `bun-types`/`@types/bun`: hoisting `bun-types` into the root `node_modules`
 * breaks sibling packages that still compile with TypeScript 4.x. The matcher surface is loose on
 * purpose — the suite runs under `bun test`, which needs no types.
 */
