...
// ---------------------------------------------------------------------------
// Shared execution-plan walker for every command-set encoder.
//
...
// deadline)` calldata. Together with the per-set codecs (`ur20.ts`, `ur21.ts`)
// it is the *only* place in the SDK that knows command bytes, sentinel
// addresses and v4 action layouts, and it decides nothing about custody: every
// recipient, payer and amount is read off the plan.
//
// WHY A SHARED WALKER AND PER-SET CODECS. The `ur-2.0` and `ur-2.1` command
// sets agree on every command byte, every v4 action byte, and every custody
// rule; they differ ONLY in the ABI layout of the three exact-in swap payloads
// (2.1 adds a `minHopPriceX36` per-hop-floor field to each — see `ur21.ts` for
// the sources and the on-chain proof). Duplicating the walker per set would
// mean the custody logic — the part where a bug is a fund-loss bug — existed
// twice and could drift; parameterizing only the three payload encodings keeps
// exactly the versioned surface versioned and nothing else. Each produced
// encoder is still pinned to one immutable deployment family via
// {@link SwapPayloadCodec.commandSet}: anything else throws, because a command
// payload that moved between router versions is a fund-loss bug, not a
// compatibility warning (the deployed 2.1.1 router reverts `SliceOutOfBounds`
// on a 2.0-shaped v3 payload, and misreads a 2.0-shaped v4 struct's
// `amountOutMinimum` slot as `amountIn` — verified live on chain 4663).
//
// Correctness is established differentially: `differential.test.ts` builds the
// same trade through `@uniswap/universal-router-sdk` (pinned per command set)
// and compares calldata byte-for-byte, and the goldens files freeze the result
// so the oracle cannot drift silently. Three encodings intentionally differ
// from that SDK; each is called out at its site below and re-stated here so
...
/**
 * Universal Router command bytes — IDENTICAL across the `ur-2.0` and `ur-2.1` command sets (the 2.1
 * revision changed three payload ABIs, not the dispatch table; `ur21.ts` documents the proof).
 *
...
/** v4 router action bytes — also identical across both command sets. Asserted against `v4-sdk`'s
 * `Actions` in `ur20.test.ts` — exported for that test only, on the same terms as {@link COMMAND}
 * above (R6). */
...
// The command payloads that are byte-identical across both command sets live here; the three
// exact-in swap payloads that are NOT live in each set's codec (`ur20.ts` / `ur21.ts`).
...
/**
 * The versioned surface of a command set: the three exact-in swap payload encodings that changed
 * between `ur-2.0` and `ur-2.1`. Everything else — command bytes, action bytes, custody, sentinels,
 * conversions, permits — is shared and lives in {@link encodeExecutionPlanFor}'s walker.
 */
export type SwapPayloadCodec = {
  /** The one command-set family the produced encoder accepts; anything else throws. */
  readonly commandSet: CommandSet
  encodeV2SwapExactIn(args: {
    recipient: Address
    amountIn: bigint
    amountOutMin: bigint
    path: Address[]
  ...
  encodeV4SwapExactIn(args: { currencyIn: Address; path: V4PathKey[]; amountIn: bigint; amountOutMinimum: bigint }): Hex
}

/** The shape every command-set encoder shares — what `encoderFor` (`./index.ts`) dispatches to. */
export type PlanEncoder = (plan: ExecutionPlan, deployment: UniversalRouterDeployment, deadline: bigint) => EncodedTx
...
function encodeV4PathKeys(legs: RouteLeg[]): V4PathKey[] {
  return legs.map((leg) => {
...
/**
 * Builds the {@link PlanEncoder} for one command set from its {@link SwapPayloadCodec}.
 *
 * The produced encoder throws {@link UnsupportedRouteError} for any deployment outside the codec's
 * command set, for any plan that fails {@link assertPlanInvariants}, and for any plan shape the
 * closed set does not cover (more than two swap groups after v2 merging, or a leading unwrap that is
 * not funded by a Permit2 pull).
 *
 ...
 */
export function encodeExecutionPlanFor(codec: SwapPayloadCodec): PlanEncoder {
  return function encodeExecutionPlan(
    plan: ExecutionPlan,
    deployment: UniversalRouterDeployment,
    deadline: bigint,
  ): EncodedTx {
    if (deployment.commandSet !== codec.commandSet)
      throw new UnsupportedRouteError(
        `unsupported Universal Router command set '${String(deployment.commandSet)}'; this encoder is pinned to '${codec.commandSet}'`,
      )
    ...
    for (const op of operations) {
      ...
      if (op.kind === 'v2-swap') {
        commands.push({
          ...
          input: codec.encodeV2SwapExactIn({
            recipient,
            amountIn,
            amountOutMin,
            path: encodeV2Path(op.legs),
            payerIsUser: payer,
          }),
        })
        ...
        commands.push({
          ...
          input: codec.encodeV3SwapExactIn({
            recipient,
            amountIn,
            amountOutMin,
            path: encodeV3Path(op.legs, deployment.wrappedNative),
            payerIsUser: payer,
          }),
        })
...
function encodeV4Swap(
  ...
  const swap = (swapAmountIn: bigint): Hex =>
    codec.encodeV4SwapExactIn({ currencyIn, path, amountIn: swapAmountIn, amountOutMinimum: amountOutMin })
