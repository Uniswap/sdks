...
| `inconclusive` | both | The search did **not** complete — abort, provider failure, or pruning got in the way. Provider trouble is always `inconclusive`, never `no-route`. It is an *incomplete* search, not an empty one: whatever it did find comes back with it — `best` and (for swaps, when one was compiled) `tx`, both optional because the search may have been cut off before finding anything, or have found only routes the chain rejected. A candidate that **reverted** in preflight is never the `best` here: it is demoted into `alternatives` as `execution: 'failed'` (with its `revertData`) exactly as on the `no-route` path, and no `tx` is offered for it — a revert stays authoritative however incomplete the rest of the search was. |
